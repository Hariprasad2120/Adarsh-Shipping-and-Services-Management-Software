...
| # | Finding | Sev | Layer | Remediation |
|---|---|---|---|---|
| C1 | **No real module registry.** Two half-systems: (a) `src/lib/app-edition.ts` — a global `APP_EDITION` env var (`full`\|`cha`) with **hardcoded** `CHA_BLOCKED_ROUTE_PREFIXES` / `CHA_VISIBLE_NAV_SECTIONS` / `CHA_BLOCKED_API_PREFIXES` arrays (the `if (edition === …)` anti-pattern); (b) `src/modules/core/organisation/module-settings.ts` + `module-config.ts` — a per-org enabled-modules / enabled-features list in `SystemSetting`, driven by a central hardcoded `TOGGLEABLE_MODULE_SECTION_IDS` / `MANAGED_FEATURE_IDS`. Neither has per-module manifests, dependency resolution, or module-contributed nav/permissions/setup steps. | P0 | core | Build `src/modules/core/module-registry/`: each module declares `{ id, name, version, dependsOn[], permissions[], nav[], settingsSchema, setupSteps[] }`. Fold `module-settings.ts` (b) into it (keep the `SystemSetting` persistence or move to `OrgModule`). Middleware/nav read the registry, not literal arrays. `APP_EDITION` (a) becomes a provisioning template, not runtime logic. |
