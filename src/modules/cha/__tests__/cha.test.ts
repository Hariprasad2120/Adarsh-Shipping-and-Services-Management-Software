...
describe("Customs House Agent (CHA) Module Integration Tests", () => {
  ...
  it("12. should verify visual filing workflow and Section 49 lifecycle", async () => {
    ...
    const importJobType = await db.chaJobType.create({
      data: {
        orgId: org.id,
        name: `Import Clearance Legacy Upload ${Date.now()}`,
      },
    });
  ...
  it("12.2. should count legacy checklist-backed document uploads toward checklist completion", async () => {
    await db.filingWorkflowInstance.deleteMany({ where: { job: { orgId: org.id } } });
    await db.filingWorkflowTemplate.deleteMany({ where: { orgId: org.id } });
    const importJobType = await db.chaJobType.create({
      data: {
    ...
    const workflowDraft = await chaService.saveFilingWorkflowDraft(ownerUser.id, org.id, null, {
      name: `Legacy Checklist Upload Bridge ${Date.now()}`,
      description: "Verifies document-lane uploads satisfy legacy checklist upload validation",
      nodes: [
        {
          key: "legacy_upload_step",
          name: "Legacy Upload Step",
          description: "Upload through the document card and complete the stage",
          category: "CHECKLIST_ITEM",
          positionX: 100,
          positionY: 100,
          isStart: true,
    ...
    const job = await chaService.createJob(ownerUser.id, org.id, {
      jobNumber: `CHA-FILING-LEGACY-UPLOAD-${Date.now()}`,
      title: "Legacy checklist upload bridge job",
      customerId: customer.id,
      jobTypeId: importJobType.id,
      branchId: branch.id,
      priority: "MEDIUM",
    ...
    const documentRequirementKey =
      activeRun.node.documentRequirementsJson?.[0]?.key ??
      activeRun.node.documentRequirements?.[0]?.key;
    expect(documentRequirementKey).toBeTruthy();

    const uploaded = await chaService.uploadFilingAttachment(
      ownerUser.id,
      org.id,
      job.id,
      activeRun.id,
    ...
    expect(uploaded.documentRequirementKey).toBe(documentRequirementKey);
    expect(uploaded.checklistItemId).toBe(activeRun.node.checklistItems[0].id);

    await chaService.completeFilingNode(
      ownerUser.id,
      org.id,
      job.id,
      activeRun.id,
      {
        checklistItemResponses: [
          {
            checklistItemId: activeRun.node.checklistItems[0].id,
            isChecked: true,
          },
        ],
      },
    ...
    const completed = await chaService.getFilingWorkflowInstance(org.id, job.id);
    expect(completed.currentNodeKey ?? null).toBeNull();
    const finishedRun = completed.nodeRuns.find((run: any) => run.id === activeRun.id);
    expect(finishedRun?.status).toBe("COMPLETED");
  }, 30000);

  it("12.3. should block transitions into prerequisite-locked filing stages", async () => {
    await db.filingWorkflowInstance.deleteMany({ where: { job: { orgId: org.id } } });
    await db.filingWorkflowTemplate.deleteMany({ where: { orgId: org.id } });
    const importJobType = await db.chaJobType.create({
      data: {
        orgId: org.id,
        name: `Import Clearance Prereq Lock ${Date.now()}`,
      },
    });

    const workflowDraft = await chaService.saveFilingWorkflowDraft(ownerUser.id, org.id, null, {
      name: `Prerequisite Lock ${Date.now()}`,
      description: "Verifies locked next-stage prerequisites are enforced at runtime",
      nodes: [
        {
          key: "ooc",
          name: "OOC",
          ...
          isStart: true,
          checklistItems: [
            { label: "OOC", isMandatory: true, allowsUpload: false },
          ],
          photoRequirements: [],
        },
        {
          key: "duty",
          name: "Duty",
          description: "Must be completed before delivery",
          category: "CHECKLIST_ITEM",
          positionX: 320,
          positionY: 100,
          checklistItems: [
            { label: "Duty", isMandatory: true, allowsUpload: false },
          ],
          photoRequirements: [],
        },
        {
          key: "delivery",
          name: "Delivery",
          description: "Locked until duty is complete",
          category: "CHECKLIST_ITEM",
          positionX: 540,
          positionY: 100,
          checklistItems: [
            { label: "Delivery", isMandatory: true, allowsUpload: false },
          ],
          photoRequirements: [],
          actionConfigJson: {
            prerequisiteGate: {
      ...
      edges: [
        { sourceKey: "ooc", targetKey: "duty", label: "Duty" },
        { sourceKey: "ooc", targetKey: "delivery", label: "Delivery" },
        { sourceKey: "duty", targetKey: "delivery", label: "Delivery" },
      ],
    });
    ...
    const job = await chaService.createJob(ownerUser.id, org.id, {
      jobNumber: `CHA-FILING-PREREQ-LOCK-${Date.now()}`,
      title: "Prerequisite lock job",
      customerId: customer.id,
      jobTypeId: importJobType.id,
      branchId: branch.id,
      priority: "MEDIUM",
    ...
    const started = await chaService.startFilingWorkflow(ownerUser.id, org.id, job.id);
    const activeRun = started.nodeRuns.find((run: any) => run.status === "ACTIVE");
    expect(activeRun?.nodeKey).toBe("ooc");
    ...
    ).rejects.toThrow(/blocked until prerequisite stages are completed: Duty/i);
  }, 30000);
  ...
  it("13.0. should normalize legacy photo requirements and checklist uploads into unified node document requirements", async () => {
    const draft = await chaService.saveFilingWorkflowDraft(ownerUser.id, org.id, null, {
      name: `Unified Upload Normalization ${Date.now()}`,
      description: "Legacy upload migration coverage",
      nodes: [
        {
          key: "photo_legacy",
          name: "Photo Legacy",
          description: "Legacy photo upload node",
          category: "Operations",
          positionX: 100,
          positionY: 100,
          isStart: true,
        ...
        {
          key: "checklist_legacy",
          name: "Checklist Legacy",
          description: "Legacy checklist upload node",
          category: "Operations",
          positionX: 340,
          positionY: 100,
          checklistItems: [
      ...
      edges: [{ sourceKey: "photo_legacy", targetKey: "checklist_legacy", label: "Next" }],
    });

    const details = await chaService.getFilingWorkflowDetails(ownerUser.id, org.id, draft.id);
    const version = details.versions?.[0];
    const photoNode = version?.nodes.find((node: any) => node.key === "photo_legacy");
    const checklistNode = version?.nodes.find((node: any) => node.key === "checklist_legacy");

    expect(photoNode?.documentRequirementsJson).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "CE/Lab Report",
          minUploads: 1,
    ...
    expect(checklistNode?.documentRequirementsJson).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "E-Way Bill",
          minUploads: 1,
