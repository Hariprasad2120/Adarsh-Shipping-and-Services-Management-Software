export type NotificationVariant =
  | "secondary"
  | "primary"
  | "destructive"
  | "success"
  | "info"
  | "mono"
  | "warning";

export type NotificationAppearance = "solid" | "outline" | "light" | "stroke";
export type NotificationPriority = "normal" | "important";
export type NotificationSource = "AMS" | "Attendance" | "HRMS" | "Admin" | "System" | "To-Do" | "CHA";

export type NotificationPolicy = {
  source: NotificationSource;
  variant: NotificationVariant;
  appearance: NotificationAppearance;
  priority: NotificationPriority;
  requiresAck: boolean;
  resendable: boolean;
  allowDismiss: boolean;
  autoFadeMs: number | null;
  emailByDefault: boolean;
  labels?: {
    open?: string;
    acknowledge?: string;
  };
};

const DEFAULT_POLICY: NotificationPolicy = {
  source: "System",
  variant: "secondary",
  appearance: "light",
  priority: "normal",
  requiresAck: false,
  resendable: true,
  allowDismiss: true,
  autoFadeMs: 5000,
  emailByDefault: false,
  labels: {
    open: "Open",
    acknowledge: "Acknowledge",
  },
};

export const NOTIFICATION_POLICIES: Record<string, NotificationPolicy> = {
  APPRAISAL_DUE: {
    source: "AMS",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "Open appraisal" },
  },
  REVIEWER_ASSIGNED: {
    source: "AMS",
    variant: "primary",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Review now", acknowledge: "Acknowledge" },
  },
  REVIEWER_UNAVAILABLE: {
    source: "AMS",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Reassign reviewer" },
  },
  REVIEWERS_PENDING_PAST_DEADLINE: {
    source: "AMS",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Review pending appraisals" },
  },
  SELF_ASSESSMENT_OPEN: {
    source: "AMS",
    variant: "primary",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Open self assessment", acknowledge: "Acknowledge" },
  },
  REVIEW_OPEN: {
    source: "AMS",
    variant: "info",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Open review", acknowledge: "Acknowledge" },
  },
  MANAGEMENT_REVIEW_OPEN: {
    source: "AMS",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Claim appraisal", acknowledge: "Acknowledge" },
  },
  MEETING_PENDING: {
    source: "AMS",
    variant: "info",
    appearance: "outline",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Confirm meeting" },
  },
  REVIEWER_AVAILABLE: {
    source: "AMS",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: false,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "View appraisal" },
  },
  SELF_ASSESSMENT_SUBMITTED: {
    source: "AMS",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: false,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "View appraisal" },
  },
  MEETING_CONFIRMED: {
    source: "AMS",
    variant: "success",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "View meeting", acknowledge: "Acknowledge" },
  },
  HIKE_FINALISED: {
    source: "AMS",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View appraisal" },
  },
  LEAVE_REQUEST_SUBMITTED: {
    source: "Attendance",
    variant: "info",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Review leave request" },
  },
  LEAVE_DECISION: {
    source: "Attendance",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View leave status" },
  },
  TODO_REMINDER: {
    source: "To-Do",
    variant: "warning",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: false,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "Open task" },
  },
  BIOMETRIC_OFFLINE: {
    source: "Attendance",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "View status", acknowledge: "Acknowledge" },
  },
  CRM_LEAD_FOLLOWUP: {
    source: "System",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: false,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "View Lead", acknowledge: "Acknowledge" },
  },
  CHA_JOB_ASSIGNED: {
    source: "CHA",
    variant: "primary",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Open Job", acknowledge: "Acknowledge" },
  },
  CHA_CHECKLIST_SUBMITTED: {
    source: "CHA",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "Review Checklist" },
  },
  CHA_CHECKLIST_APPROVED: {
    source: "CHA",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "Open Job" },
  },
  CHA_REWORK_REQUESTED: {
    source: "CHA",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Fix Checklist", acknowledge: "Acknowledge" },
  },
  CHA_FILING_DUE: {
    source: "CHA",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "File Bill" },
  },
  CHA_FILING_OVERDUE: {
    source: "CHA",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "File Bill", acknowledge: "Acknowledge" },
  },
  CHA_DELIVERY_ORDER_VALIDITY_EXPIRING: {
    source: "CHA",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Go To Additional Data Page", acknowledge: "Acknowledge" },
  },
  CHA_DELIVERY_ORDER_VALIDITY_EXPIRED: {
    source: "CHA",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Go To Additional Data Page", acknowledge: "Acknowledge" },
  },
  CHA_SECTION49_VALIDITY_EXPIRING: {
    source: "CHA",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Open Documents", acknowledge: "Acknowledge" },
  },
  CHA_SECTION49_VALIDITY_EXPIRED: {
    source: "CHA",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Open Documents", acknowledge: "Acknowledge" },
  },
  CHA_FILING_WORKFLOW_NODE: {
    source: "CHA",
    variant: "info",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Open Job" },
  },
  CHA_EXPENSE_SUBMITTED: {
    source: "CHA",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "Review Expense" },
  },
  CHA_EXPENSE_APPROVED: {
    source: "CHA",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "Open Expenses" },
  },
  CHA_EXPENSE_READY: {
    source: "CHA",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "Open Expenses" },
  },
  CHA_EXPENSE_PAID: {
    source: "CHA",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "Open Expenses" },
  },
  CHA_JOB_DELETION_REQUESTED: {
    source: "CHA",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Review Request", acknowledge: "Acknowledge" },
  },
  CHA_JOB_DELETION_REJECTED: {
    source: "CHA",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Open Job" },
  },
  CHA_JOB_DELETED: {
    source: "CHA",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Open Jobs" },
  },

  // ── HRMS Mobile Attendance & Tracking ─────────────────────────────────────

  HRMS_ON_DUTY_SUBMITTED: {
    source: "HRMS",
    variant: "info",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "Review Request" },
  },
  HRMS_ON_DUTY_APPROVED: {
    source: "HRMS",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View Request" },
  },
  HRMS_ON_DUTY_REJECTED: {
    source: "HRMS",
    variant: "destructive",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View Request" },
  },
  HRMS_ON_DUTY_STARTED: {
    source: "HRMS",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: false,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "View Trip" },
  },
  HRMS_ON_DUTY_COMPLETED: {
    source: "HRMS",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "View Summary" },
  },
  HRMS_TRACKING_OFFLINE: {
    source: "HRMS",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "View Status", acknowledge: "Acknowledge" },
  },
  HRMS_GPS_DISABLED: {
    source: "HRMS",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: false,
    labels: { open: "View Status", acknowledge: "Acknowledge" },
  },
  HRMS_FACE_AUTH_FAILED: {
    source: "HRMS",
    variant: "destructive",
    appearance: "light",
    priority: "important",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "View Details" },
  },
  HRMS_REIMBURSEMENT_SUBMITTED: {
    source: "HRMS",
    variant: "info",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: false,
    labels: { open: "Review Claim" },
  },
  HRMS_REIMBURSEMENT_APPROVED: {
    source: "HRMS",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View Claim" },
  },
  HRMS_REIMBURSEMENT_REJECTED: {
    source: "HRMS",
    variant: "destructive",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View Claim" },
  },
  HRMS_REIMBURSEMENT_PAID: {
    source: "HRMS",
    variant: "success",
    appearance: "light",
    priority: "normal",
    requiresAck: false,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: 5000,
    emailByDefault: true,
    labels: { open: "View Claim" },
  },
  HRMS_AGREEMENT_UPDATED: {
    source: "HRMS",
    variant: "warning",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: false,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "Review Agreement", acknowledge: "Acknowledge" },
  },
  HR_LETTER_ISSUED: {
    source: "HRMS",
    variant: "primary",
    appearance: "light",
    priority: "important",
    requiresAck: true,
    resendable: true,
    allowDismiss: true,
    autoFadeMs: null,
    emailByDefault: true,
    labels: { open: "View Letter", acknowledge: "Acknowledge" },
  },
};

export function getNotificationPolicy(kind: string): NotificationPolicy {
  return NOTIFICATION_POLICIES[kind] ?? DEFAULT_POLICY;
}
