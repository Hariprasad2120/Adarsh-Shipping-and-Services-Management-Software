...
export function WorkReportsView() {
  ...
  const loadReports = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/hrms/peopleplus/work-reports?filter=${filter}`);
  ...
  const handleCreateReport = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/hrms/peopleplus/work-reports", {
        method: "POST",
  ...
  const handleApprovalAction = async (reportId: string, status: "APPROVED" | "REJECTED") => {
    try {
      const res = await fetch(`/api/hrms/peopleplus/work-reports/${reportId}/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
