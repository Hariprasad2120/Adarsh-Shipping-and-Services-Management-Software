...
import { ArrowLeft, ArrowRight, Filter, MoreHorizontal, Plane, Plus, Ship } from "lucide-react";
import {
            ...
            <OperationalDataTable>
              <OperationalDataTableHeader
                ...
                actions={
                  <>
                    ...
                    <OperationalFilterButton activeCount={2}>
              ...
              <OperationalDataTableWrap>
                <OperationalTable>
                  <thead>
                    <tr>
                      <OperationalTableHead><NeonCheckbox aria-label="Select all" /></OperationalTableHead>
                  ...
                  <tbody>
                    {shipments.filter(row => filter === "All" || row[2].includes(filter)).map((row, i) => (
                      <tr key={row[0]}>
                        <OperationalTableCell><NeonCheckbox aria-label={`Select ${row[0]}`} /></OperationalTableCell>
                        <OperationalPrimaryCell primary={row[0]} secondary={i === 0 ? "Created today" : `${i + 1} days ago`} />
                        <OperationalTableCell>{row[1]}</OperationalTableCell>
                        <OperationalTableCell>
                          <OperationalMode icon={row[2].includes("Sea") ? <Ship size={13} /> : <Plane size={13} />}>
                            {row[2]}
                          </OperationalMode>
                        </OperationalTableCell>
                        <OperationalTableCell>{row[3]}</OperationalTableCell>
                        <OperationalTableCell>
                          <OperationalStatus tone={i === 1 ? "warning" : "success"}>{row[4]}</OperationalStatus>
                        </OperationalTableCell>
                        <OperationalTableCell>
                          <OperationalRowAction><MoreHorizontal size={16} aria-hidden="true" /></OperationalRowAction>
                        </OperationalTableCell>
                      </tr>
                    ))}
                  </tbody>
              ...
              <OperationalDataTableFooter summary="Showing 1-3 of 284 jobs">
                <OperationalPagination>
                  <OperationalPageButton disabled><ArrowLeft size={13} aria-hidden="true" /></OperationalPageButton>
                  <OperationalPageButton active>1</OperationalPageButton>
                  <OperationalPageButton>2</OperationalPageButton>
                  <OperationalPageButton>3</OperationalPageButton>
                  <OperationalPageButton><ArrowRight size={13} aria-hidden="true" /></OperationalPageButton>
                </OperationalPagination>
              </OperationalDataTableFooter>
            </OperationalDataTable>
          </section>
