...
export function VendorNoteDraftsClient({
  notes,
}: {
  notes: Array<{
    id: string;
  ...
  async function submitDraft(id: string) {
    ...
    try {
      ...
      toast.success("Vendor debit note submitted for canonical approval");
      router.refresh();
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Vendor debit note could not be submitted",
      );
    } finally {
      setBusyId(null);
    }
      ...
      <tbody>
        {notes.length === 0 ? (
          <AccountingEmptyTableRow colSpan={7}>
            No vendor debit-note drafts are waiting for submission.
          </AccountingEmptyTableRow>
        ) : (
          notes.map((note) => (
            <tr key={note.id}>
