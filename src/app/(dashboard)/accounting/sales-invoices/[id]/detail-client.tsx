...
export function SalesInvoiceDetailClient({ invoice }: SalesInvoiceDetailClientProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);

  const handleSubmit = async () => {
    if (!confirm("Are you sure you want to submit this invoice? This will post ledger lines and record Accounts Receivable.")) return;
    setIsSubmitting(true);
    try {
      const res = await submitSalesInvoiceAction(invoice.id);
      if (res.ok) {
        toast.success("Sales Invoice submitted and posted successfully!");
        router.refresh();
      } else {
        toast.error(res.error);
      }
    } catch (err: any) {
      toast.error(err.message || "Failed to submit sales invoice");
    } finally {
  ...
  const handleCancel = async () => {
    ...
    if (!confirm("Are you sure you want to cancel this invoice? This will post reversal ledger entries.")) return;
    setIsCancelling(true);
    try {
      const res = await cancelSalesInvoiceAction(invoice.id);
      if (res.ok) {
        toast.success("Sales Invoice cancelled and reversed!");
        router.refresh();
      } else {
        toast.error(res.error);
      }
    } catch (err: any) {
      toast.error(err.message || "Failed to cancel sales invoice");
    } finally {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 p-6 rounded-xl bg-[#0f1319] border border-[#1c212a]/55 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-xs text-slate-400">
            <div>
              <strong className="text-white block mb-0.5">Customer / Client:</strong>
              {invoice.customer?.name || "—"}
            </div>
            <div>
              <strong className="text-white block mb-0.5">Billing Address:</strong>
              <span className="whitespace-pre-line leading-relaxed">{invoice.customer?.billingAddress || "No billing address"}</span>
            </div>
            <div>
