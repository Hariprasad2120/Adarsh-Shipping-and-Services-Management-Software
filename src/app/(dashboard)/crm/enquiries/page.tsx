...
export default async function CrmEnquiriesPage({
  ...
  try {
    ...
    return (
      <CrmPermissionState description="You do not have permission to view CRM enquiries." />
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--mnx-border)]/30 pb-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-mono-muted">
            Manage active shipping enquiries, perishable cargo logs, and
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Link
          ...
          className={`p-6 rounded-xl bg-[var(--mnx-surface)] border-2 transition-all duration-200 block cursor-pointer ${
            type === "all"
              ? "border-[var(--mnx-accent)] mnx-shadow-panel translate-y-[2px] translate-x-[2px]"
              : "border-[var(--mnx-border)] hover:border-[var(--mnx-accent)]/40 mnx-shadow-panel hover:translate-y-[-2px] hover:translate-x-[-2px] "
        ...
        >
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              ...
              <span className="text-3xl font-bold text-mono-text block mnx-numeric">
                {allEnquiriesCount}
              </span>
            </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center justify-between bg-[var(--mnx-surface)] p-4 rounded-xl border border-[var(--mnx-border)]/50">
        ...
        <div className="flex flex-wrap gap-3 w-full lg:w-auto">
          ...
          <Link
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border-2 cursor-pointer ${
              type === "perishable"
                ? "bg-[var(--mnx-accent)] text-mono-text border-[var(--mnx-accent)] shadow-none translate-y-[2px] translate-x-[2px]"
                : "bg-[var(--mnx-surface)] text-[var(--mnx-accent)] border-[var(--mnx-accent)]/40 hover:border-[var(--mnx-accent)]/85 mnx-shadow-panel hover:translate-y-[-1px] hover:translate-x-[-1px]  active:translate-y-[2px] active:translate-x-[2px] active:shadow-none"
            }`}
          >
            Perishables Only
          </Link>
        ...
        <form
          method="GET"
          className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto lg:flex-1 justify-end"
        >
                    ...
                    <tr key={enquiry.id} className="mnx-row-link">
                      ...
                      <td className="px-6 py-4">
                        <div className="font-medium">
                          {enquiry.firstName ? `${enquiry.firstName} ` : ""}
                          {enquiry.lastName}
                        </div>
                        ...
                        {isSea ? (
                          <div className="space-y-0.5">
                            ...
                            <span className="text-[11px] text-mono-muted block">
                              {details.commodity} • {details.weight} •{" "}
                              {details.seaLclFcl}
                            </span>
                          </div>
                        ) : isAir ? (
                          ...
                          <span className="text-mono-muted italic text-xs">
                      ...
                      <td className="px-6 py-4 space-y-1">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {enquiry.isFutureFollowUp && (
                            <span
                              className="px-2 py-0.5 rounded text-[9px] font-bold bg-[var(--mnx-accent)]/10 text-[var(--mnx-accent)] border border-[var(--mnx-accent)]/20 uppercase flex items-center gap-1"
                              title={
                                enquiry.followUpReminderDate
                                  ? `Remind at: ${new Date(enquiry.followUpReminderDate).toLocaleString("en-IN")}`
                                  : ""
                              }
                            >
