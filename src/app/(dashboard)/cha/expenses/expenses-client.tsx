...
function getFuelRowDistanceKm(row: FuelExpenseRow) {
  const oneWayKm =
    row.mode === "KM" ? Number(row.km) || 0 : (row.routeDistanceKm ?? 0);
  return Math.max(0, row.roundTrip ? oneWayKm * 2 : oneWayKm);
}

function getFuelRowAmount(row: FuelExpenseRow) {
  return (
    Math.round(getFuelRowDistanceKm(row) * FUEL_POLICY_RATE_PER_KM * 100) / 100
  );
}
...
function createRouteSnapshotFile(row: FuelExpenseRow, fileName: string) {
  ...
  const points = coords
    .map((point) => {
      const x =
        padding + ((point.lon - minLon) / lonSpan) * (width - padding * 2);
      const y =
        height -
        padding -
    ...
    .join(" ");
  const escapedFrom = row.fromAddress.replace(
    /[&<>"']/g,
    (char) =>
      ({
        "&": "&amp;",
  ...
  const escapedTo = row.toAddress.replace(
    /[&<>"']/g,
    (char) =>
      ({
        "&": "&amp;",
...
function LocationSearchInput({
  ...
  return (
    <div className="relative space-y-1">
      <label htmlFor={id} className="mnx-label">
        {label}
      </label>
      <Input
        id={id}
      ...
      />
      {searching ? (
        <p className="text-xs mnx-text-muted">Searching OpenStreetMap...</p>
      ) : null}
      {searchError ? (
...
export function ExpensesClient({
  ...
  const [directApprovalRoute, setDirectApprovalRoute] = useState<
    "MANAGER_THEN_ACCOUNTS" | "DIRECT_ACCOUNTS"
  >("MANAGER_THEN_ACCOUNTS");
  ...
  const [fuelRows, setFuelRows] = useState<FuelExpenseRow[]>(() => [
    createFuelExpenseRow(),
  ]);
  const [routeLoadingRowId, setRouteLoadingRowId] = useState<string | null>(
    null,
  );
  const [routePlannerRowId, setRoutePlannerRowId] = useState<string | null>(
    null,
  );
  ...
  const [clarificationRequestId, setClarificationRequestId] = useState<
    string | null
  >(null);
  ...
  const pendingCount = initialExpenses.filter((req) =>
    ["UNDER_REVIEW", "ACCOUNTS_REVIEW", "CLARIFICATION_REQUIRED"].includes(
      req.status,
    ),
  ).length;
  ...
  const handleReviewDecision = async (
    ...
    if (
      ...
      toast.error(
        "Review remarks are required for rejections or clarification requests.",
      );
      return;
    }
  ...
  const selectedDirectJob = jobOptions.find(
    (job) =>
      job.jobNumber.toLowerCase() === directJobQuery.trim().toLowerCase(),
  );
  const fuelTotalKm = fuelRows.reduce(
    (total, row) => total + getFuelRowDistanceKm(row),
    0,
  );
  const fuelTotalAmount = fuelRows.reduce(
    (total, row) => total + getFuelRowAmount(row),
    0,
  );
  ...
  const captureRouteDistance = async (
    rowId: string,
    overrides: Partial<
      Pick<FuelExpenseRow, "fromAddress" | "toAddress" | "stopAddresses">
    > = {},
  ) => {
    ...
    const waypoints = routeRow.stopAddresses
      .map((stop) => stop.trim())
      .filter(Boolean);
    if (!origin || !destination) return;
    ...
    updateFuelRow(rowId, {
      routeDistanceStatus: "Capturing route distance with free OSRM routing...",
    });
    try {
      const routeLocations = await Promise.all(
        [origin, ...waypoints, destination].map((address) =>
          geocodeAddress(address),
        ),
      );
      const coordinatePath = routeLocations
        .map((location) => `${location.lon},${location.lat}`)
        .join(";");
      const routeResponse = await fetch(
        `${OSRM_ROUTE_URL}/${coordinatePath}?overview=full&geometries=geojson&steps=false`,
      );
      if (!routeResponse.ok)
        throw new Error(
          "OpenStreetMap routing could not calculate this route.",
        );
      ...
      updateFuelRow(rowId, {
        ...
        mapSnapshotFile: null,
        routeDistanceStatus:
          error?.message || "Free routing could not calculate this route.",
      });
    } finally {
    ...
    <div className="space-y-4">
      <div className="space-y-3 rounded-xl border mnx-border mnx-bg-surface p-3">
          ...
          <Button
            ...
            onClick={() =>
              updateFuelRow(row.id, {
                stopAddresses: [...row.stopAddresses, ""],
              })
            }
          >
        ...
        <LocationSearchInput
          ...
          onPlaceSelected={(suggestion) =>
            updateFuelRow(row.id, { fromAddress: suggestion.displayName })
          }
        />
        {row.stopAddresses.map((stopAddress, stopIndex) => (
          <div
            key={`${row.id}-stop-${stopIndex}`}
          ...
          >
            <LocationSearchInput
              ...
              onValueChange={(value) =>
                updateFuelRow(row.id, {
                  stopAddresses: row.stopAddresses.map((item, index) =>
                    index === stopIndex ? value : item,
                  ),
                  routeDistanceKm: null,
                  routeCoordinates: [],
                  mapSnapshotFile: null,
                  routeDistanceStatus:
                    "Capture KM after entering route details.",
                })
              }
              onPlaceSelected={(suggestion) =>
                updateFuelRow(row.id, {
                  stopAddresses: row.stopAddresses.map((item, index) =>
                    index === stopIndex ? suggestion.displayName : item,
                  ),
                })
              }
            />
            <Button
              ...
              onClick={() =>
                updateFuelRow(row.id, {
                  stopAddresses: row.stopAddresses.filter(
                    (__, index) => index !== stopIndex,
                  ),
                  routeDistanceKm: null,
                  routeCoordinates: [],
                  mapSnapshotFile: null,
        ...
        <LocationSearchInput
          ...
          onPlaceSelected={(suggestion) =>
            updateFuelRow(row.id, { toAddress: suggestion.displayName })
          }
        />
        <div className="rounded-xl border mnx-border mnx-bg-soft p-3">
          ...
          <p className="text-sm mnx-text-primary">
            {row.routeDistanceKm
              ? `${row.routeDistanceKm.toLocaleString("en-IN")} one-way KM`
              : "Not captured"}
          </p>
        ...
        <div className="rounded-xl border mnx-border mnx-bg-surface p-3">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs mnx-text-muted">
                Uses OpenStreetMap address search and free OSRM driving
                distance. It does not require a billing account.
              </p>
            </div>
            <Button
              type="button"
        ...
        <div className="rounded-xl border mnx-border mnx-bg-surface p-3">
          ...
          <p className="mt-1 text-xs mnx-text-muted">
            Opens Google Maps without using paid APIs. Enter the one-way KM
            shown by Google Maps here.
          </p>
        ...
        {row.mapSnapshotFile ? (
          <div className="rounded-xl border mnx-border mnx-bg-soft p-3">
            ...
            <p className="mt-1 text-xs mnx-text-primary">
              {row.mapSnapshotFile.type === "image/svg+xml"
                ? "Generated route snapshot"
                : "Uploaded route evidence"}
  ...
  const handleCreateDirectExpense = async (e: React.FormEvent) => {
    ...
    if (directScope === "JOB" && !selectedDirectJob) {
      toast.error(
        "Choose a valid job number from the suggestions or select Other.",
      );
      return;
    }
    ...
    if (raiseView === "GENERAL") {
      ...
      if (invalidFuelRow) {
        toast.error(
          "Enter purpose, date, and valid distance for every fuel expense.",
        );
        return;
      }
    }
    ...
    try {
      ...
      if (directScope === "JOB" && selectedDirectJob)
        formData.set("jobId", selectedDirectJob.id);
      formData.set("expenseScope", directScope);
      formData.set(
        "directPurpose",
        directScope === "OTHER" ? directOtherPurpose : "",
      );
        ...
        formData.set(
          ...
          JSON.stringify(
            fuelRows.map((row, index) => {
              const distanceKm = getFuelRowDistanceKm(row);
              const stopsLabel = row.stopAddresses
                .map((stop) => stop.trim())
                .filter(Boolean)
                .join(" via ");
              const routeLabel =
      ...
      if (raiseView === "FUEL") {
        fuelRows.forEach((row, rowIndex) => {
          if (row.mapSnapshotFile)
            formData.append(
              `receiptAttachment:${rowIndex}`,
              row.mapSnapshotFile,
            );
          row.receiptFiles.forEach((file) =>
            formData.append(`receiptAttachment:${rowIndex}`, file),
          );
        });
      } else {
        directLineReceiptFiles.forEach((file) =>
          formData.append("receiptAttachment:0", file),
        );
      }
      const res =
        await actions.createDirectExpenseRequestWithAttachmentAction(formData);

      if (res.ok) {
        toast.success(
          directApprovalRoute === "DIRECT_ACCOUNTS"
            ? "Expense request sent to Accounts."
            : "Expense request sent for manager review.",
        );
  ...
  const activeRoutePlannerRow =
    fuelRows.find((row) => row.id === routePlannerRowId) ?? null;
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <div className="mnx-bg-surface mnx-border mnx-border-accent rounded-xl border mnx-border mnx-bg-surface p-4">
            ...
            <p className="mt-2 text-xl mnx-text-primary mnx-numeric">
          ...
          <section className="rounded-xl border mnx-border mnx-bg-surface shadow-sm">
            <div className="flex items-center justify-between gap-3 p-5">
              <div className="grid min-w-0 grid-cols-[4px_minmax(0,1fr)] items-center gap-3">
                <span
                  className="h-8 w-1 rounded-sm mnx-bg-accent-soft"
                  aria-hidden="true"
                />
                <div className="min-w-0">
                  ...
                  <p className="text-xs mnx-text-muted">
                    Submit against a CHA job or choose Other for office
                    expenses.
                  </p>
                </div>
              </div>
            ...
            <form
              onSubmit={handleCreateDirectExpense}
              className="border-t mnx-border p-5"
            >
              <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
                <div className="space-y-4">
                  <div className="inline-flex rounded-xl border mnx-border mnx-bg-surface p-1">
                    <Button
                      type="button"
                      variant={raiseView === "GENERAL" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setRaiseView("GENERAL")}
                    >
                      General Expense
                    </Button>
                    <Button
                      type="button"
                      variant={raiseView === "FUEL" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setRaiseView("FUEL")}
                    >
                      Fuel Expenses
                    </Button>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      type="button"
                      variant={directScope === "JOB" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setDirectScope("JOB")}
                    >
                      Job Expense
                    </Button>
                    <Button
                      type="button"
                      variant={directScope === "OTHER" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setDirectScope("OTHER")}
                    >
                      Other
                    </Button>
                  </div>
                    ...
                    <div className="space-y-1">
                      ...
                      {directJobQuery && !selectedDirectJob ? (
                        <p className="text-xs text-tertiary">
                          Select an existing job number from the suggestions.
                        </p>
                      ) : null}
                    </div>
                  ) : (
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <DateInput
                          ...
                          onChange={(e) =>
                            setDirectRequiredDate(e.target.value)
                          }
                          className="h-11 w-full text-sm"
                        />
                      </div>
                      <div className="md:col-span-2">
                        <FileUploadField
                          ...
                          onRemoveSelectedFile={(_, fileIndex) =>
                            setDirectLineReceiptFiles((current) =>
                              current.filter(
                                (__, index) => index !== fileIndex,
                              ),
                            )
                          }
                        />
                      ...
                      <div className="rounded-xl border mnx-border mnx-bg-soft p-3">
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                          <div>
                            ...
                            <p className="text-sm mnx-text-primary">
                              INR {FUEL_POLICY_RATE_PER_KM.toFixed(2)} per KM
                            </p>
                            <p className="mt-1 text-xs mnx-text-muted">
                              Free KM capture uses OSRM routing. Google Maps
                              remains available as a manual evidence fallback.
                            </p>
                          </div>
                          <div className="flex gap-4 text-xs mnx-text-muted">
                            <span className="mnx-numeric">
                              {fuelTotalKm.toLocaleString("en-IN")} KM
                            </span>
                            <span className="mnx-numeric">
                              INR {fuelTotalAmount.toLocaleString("en-IN")}
                            </span>
                          </div>
                        </div>
                      </div>
                          ...
                          >
                            <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                              <div>
                                <p className="mnx-label">
                                  Fuel Expense {index + 1}
                                </p>
                                <p className="text-xs mnx-text-muted">
                                  Distance {distanceKm.toLocaleString("en-IN")}{" "}
                                  KM • Amount INR{" "}
                              ...
                              <div className="flex flex-wrap gap-2">
                                <Button
                                  type="button"
                                  variant={
                                    row.mode === "KM" ? "default" : "outline"
                                  }
                                  size="sm"
                                  onClick={() =>
                                    updateFuelRow(row.id, { mode: "KM" })
                                  }
                                >
                                  Enter KM
                                </Button>
                                <Button
                                  type="button"
                                  variant={
                                    row.mode === "ROUTE" ? "default" : "outline"
                                  }
                                  size="sm"
                                  onClick={() =>
                                    updateFuelRow(row.id, { mode: "ROUTE" })
                                  }
                                >
                                  From / To
                                </Button>
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                ...
                                <div className="md:col-span-2 rounded-xl border mnx-border mnx-bg-surface p-3">
                                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                    <div className="min-w-0 space-y-1">
                                      ...
                                      <p className="text-xs mnx-text-muted">
                                        {row.routeDistanceKm
                                          ? `${row.routeDistanceKm.toLocaleString("en-IN")} one-way KM captured`
                                          : "Open the planner to enter route, stops, capture KM, and attach map snapshot."}
                                      </p>
                                      {row.mapSnapshotFile ? (
                              ...
                              <label className="flex cursor-pointer items-center gap-3 rounded-xl border mnx-border mnx-bg-surface p-3">
                                ...
                                />
                                <span>
                                  <span className="block text-sm font-medium mnx-text-primary">
                                    Round trip
                                  </span>
                                  <span className="text-xs mnx-text-muted">
                                    Doubles the captured one-way KM.
                                  </span>
                                </span>
                              </label>
                              <div className="md:col-span-2">
                                <FileUploadField
                                  ...
                                  selectedFiles={row.receiptFiles.map(
                                    (file) => ({
                                      file,
                                      name: file.name,
                                      sizeBytes: file.size,
                                  ...
                                  onClear={() =>
                                    updateFuelRow(row.id, { receiptFiles: [] })
                                  }
                                  onRemoveSelectedFile={(_, fileIndex) =>
                                    updateFuelRow(row.id, {
                                      receiptFiles: row.receiptFiles.filter(
                                        (__, index) => index !== fileIndex,
                                      ),
                                    })
                                  }
                ...
                <aside className="space-y-4 mnx-border xl:border-l xl:pl-5">
                  <div className="space-y-2">
                    ...
                    <label className="flex cursor-pointer items-start gap-3 rounded-xl border mnx-border mnx-bg-surface p-3">
                      <Input
                        ...
                        checked={
                          directApprovalRoute === "MANAGER_THEN_ACCOUNTS"
                        }
                        onChange={() =>
                          setDirectApprovalRoute("MANAGER_THEN_ACCOUNTS")
                        }
                      ...
                      <span>
                        <span className="block text-sm font-medium mnx-text-primary">
                          Via manager, then Accounts
                        </span>
                        <span className="text-xs mnx-text-muted">
                          Manager approves first; Accounts handles disbursement.
                        </span>
                      </span>
                    </label>
                    <label className="flex cursor-pointer items-start gap-3 rounded-xl border mnx-border mnx-bg-surface p-3">
                      <Input
                        ...
                        onChange={() =>
                          setDirectApprovalRoute("DIRECT_ACCOUNTS")
                        }
                        className="mt-1"
                      />
                      <span>
                        ...
                        <span className="text-xs mnx-text-muted">
                          Accounts can approve or send it to the manager.
                        </span>
                      </span>
                    </label>
                  </div>
                  ...
                  <label className="flex cursor-pointer items-start gap-3 rounded-xl border mnx-border mnx-bg-surface p-3">
                    ...
                    <span>
                      ...
                      <span className="text-xs mnx-text-muted">
                        Flags priority without bypassing approval.
                      </span>
                    </span>
                  </label>
                  ...
                  {directUrgent ? (
                    <div className="space-y-1">
                      <label className="mnx-label text-tertiary">
                        Urgency Reason *
                      </label>
                      <Input
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => setShowDirectRequestForm(false)}
                    >
                      Cancel
                    </Button>
          ...
          <div className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs mnx-text-muted">
                  {initialExpenses.length} records visible
                </p>
              </div>
            </div>
                  ...
                  <div className="mnx-floating-surface mnx-cha-menu mnx-cha-filter-popover absolute right-0 z-20 mt-2 w-[min(92vw,360px)] space-y-3 p-4">
                    <div className="grid gap-3">
                      <NativeSelect
                        ...
                        className="h-10 w-full text-sm font-sans"
                      >
                        ...
                        <option value="RECEIPT_ACKNOWLEDGED">
          ...
          {initialExpenses.length === 0 ? (
            <div className="m-5 rounded-xl border mnx-border mnx-bg-surface p-12 text-center mnx-text-muted shadow-sm">
              ...
              <p className="text-sm font-semibold">
                Disbursement queue is currently empty.
              </p>
              <p className="mt-1 text-xs">
                Pending expense requests from operations will appear here.
              </p>
            </div>
          ) : (
            <div className="w-full mnx-bg-surface">
              ...
              {initialExpenses.map((req) => {
                ...
                const openReviewForm = (
                  decision: "CLARIFICATION_REQUIRED" | "REJECTED",
                ) => {
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-2">
                        {canAccountsReviewThisExpense ? (
                          <>
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleRouteToManager(req.id)}
                              className="text-xs"
                            ...
                            <Button
                              size="sm"
                              onClick={() => handleAccountsApprove(req.id)}
                              className="text-xs"
                              disabled={loading !== null}
                            >
                              Approve
                        ...
                        {canReviewThisExpense ? (
                          <>
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                openReviewForm("CLARIFICATION_REQUIRED")
                              }
                              className="text-xs"
                            >
                              Clarify
                            ...
                            <Button
                              size="sm"
                              onClick={() =>
                                handleReviewDecision(req.id, "APPROVED")
                              }
                              className="text-xs"
                              disabled={loading !== null}
                            >
                        ...
                        {canMarkReadyThisExpense ? (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleReadyForDisbursement(req.id)}
                            className="text-xs"
                            disabled={loading !== null}
                          >
                            Ready
                          ...
                          <div className="min-w-0 space-y-4">
                                  ...
                                  <tbody>
                                    {req.lines.map((line: any) => (
                                      <tr key={line.id}>
                                        ...
                                        <td className="text-right mnx-numeric">
                                          ₹
                              ...
                              <section className="grid gap-3 md:grid-cols-2">
                                {req.upiNumber ? (
                                  <div className="rounded-xl border mnx-border mnx-bg-surface p-4">
                                    ...
                                    <p className="mt-1 text-sm mnx-text-primary mnx-numeric">
                                      {req.upiNumber}
                                    </p>
                                  </div>
                                ) : null}
                            ...
                            {isUrgent && req.urgencyReason ? (
                              <section className="mnx-bg-surface mnx-border mnx-border-warning rounded-xl border border-tertiary/35 mnx-bg-surface p-4 text-xs leading-relaxed">
                                <p className="mnx-label text-tertiary">
                                  Urgency Justification
                                </p>
                                <p className="mt-2 mnx-text-primary">
                                  {req.urgencyReason}
                                </p>
                              </section>
                            ) : null}
                            ...
                            {req.queries?.length ? (
                              <section className="space-y-3">
                                ...
                                {req.queries.map((q: any) => (
                                  <div
                                    key={q.id}
                                    className="rounded-xl border border-tertiary/35 mnx-bg-surface p-4 text-xs"
                                  >
                                    <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                                      <div>
                                        ...
                                        <p className="mt-1 mnx-text-primary">
                                          {q.queryText}
                                        </p>
                                    ...
                                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                      <div>
                                        <p className="font-medium mnx-text-primary">
                                          Payout completed via{" "}
                                          {payment.paymentMethod}
                                        </p>
                                        <p className="mt-1 text-[10px] mnx-text-muted">
                                          Txn ID: {payment.transactionReference}{" "}
                            ...
                            <div className="space-y-5">
                              ...
                              {canPayThisExpense && payRequestId === req.id ? (
                                <form
                                  ...
                                  className="space-y-4 rounded-xl border mnx-border-accent mnx-bg-surface p-4"
                                >
                                  <h3 className="mnx-heading-3 mnx-text-primary">
                                    Payout Details
                                  </h3>
      ...
      <Modal
        ...
        className="max-w-7xl"
      >
        {activeRoutePlannerRow
          ? renderRoutePlanner(activeRoutePlannerRow)
          : null}
      </Modal>
    </>
  );
