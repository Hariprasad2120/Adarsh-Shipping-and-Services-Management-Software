...
function slugifyFilingRequirementKey(value: string) {
  return (
    value
      .trim()
      .toLowerCase()
...
function normalizePrerequisiteGate(value: unknown) {
  const raw =
    value && typeof value === "object"
      ...
      : {};
  const requiredNodeKeys = Array.isArray(raw.requiredNodeKeys)
    ...
    : [];
  return {
    enabled: !!raw.enabled && requiredNodeKeys.length > 0,
    mode: raw.mode === "ANY" ? "ANY" : "ALL",
    requiredNodeKeys,
  };
}

function normalizeNodeActionConfig(value: unknown) {
  const raw =
    value && typeof value === "object"
      ? (value as Record<string, unknown>)
      : {};
...
function evaluateNodePrerequisiteStatus(
  ...
  const prerequisiteGate = normalizeNodeActionConfig(
...
function normalizeUnifiedNodeDocuments(
  documentRequirements: any,
  photoRequirements: any,
  checklistItems: any,
) {
  const normalizedDocuments = Array.isArray(documentRequirements)
    ? documentRequirements.map((requirement: any, index: number) => ({
        key: requirement.key || `document_${index + 1}`,
        label: requirement.label || `Document ${index + 1}`,
        description: requirement.description || "",
        required: requirement.required !== false,
        acceptedFileTypes: Array.isArray(requirement.acceptedFileTypes)
          ? requirement.acceptedFileTypes
          : ["application/pdf", "image/jpeg", "image/png"],
        minUploads: Number(requirement.minUploads ?? 1),
        maxUploads:
          requirement.maxUploads === null || requirement.maxUploads === undefined
            ? null
            : Number(requirement.maxUploads),
        allowReplacement: requirement.allowReplacement !== false,
        allowPreview: requirement.allowPreview !== false,
        requiresValidity: !!requirement.requiresValidity,
        isVisibleInTimeline: requirement.isVisibleInTimeline !== false,
      }))
    : [];
  ...
  const normalizedPhotoRequirements = Array.isArray(photoRequirements)
    ? photoRequirements.map((requirement: any, index: number) => ({
        key:
          requirement.key ||
          `photo_${slugifyFilingRequirementKey(requirement.label || `upload_${index + 1}`)}_${index + 1}`,
        label: requirement.label || `Upload ${index + 1}`,
        description: requirement.description || "",
        required: requirement.isMandatory !== false,
        acceptedFileTypes: Array.isArray(requirement.acceptedFileTypes)
          ? requirement.acceptedFileTypes
          : ["application/pdf", "image/jpeg", "image/png"],
        minUploads: Number(requirement.minPhotos ?? 1),
        maxUploads:
          requirement.maxPhotos === null || requirement.maxPhotos === undefined
            ? null
            : Number(requirement.maxPhotos),
  ...
  if (normalizedPhotoRequirements.length > 0) {
    return normalizedPhotoRequirements;
  }
  ...
  return Array.isArray(checklistItems)
    ? checklistItems
        ...
        .map((item: any, index: number) => ({
          key:
            item.key ||
            `checklist_${slugifyFilingRequirementKey(item.documentType || item.label || `upload_${index + 1}`)}_${index + 1}`,
          label: item.documentType || item.label || `Upload ${index + 1}`,
...
function mergeChecklistResponsesForActiveNode(
  ...
  for (const [itemId, localResponse] of Object.entries(localResponses)) {
    const serverResponse = merged[itemId];
    if (!serverResponse) {
      merged[itemId] = { ...localResponse };
      continue;
    }
    merged[itemId] = {
      ...serverResponse,
      isChecked: serverResponse.isChecked || !!localResponse.isChecked,
      remarks:
        localResponse.remarks?.trim() || serverResponse.remarks || "",
      fileKey: localResponse.fileKey || serverResponse.fileKey || undefined,
      delayRemarks:
        localResponse.delayRemarks?.trim() ||
...
function AdditionalDataStatCard({
  ...
  return (
    <div className="card-cyan-outline card-top-accent rounded-xl border border-outline-variant/40 bg-surface p-3 shadow-sm">
      ...
      <p
        className={`mt-1 text-sm text-on-surface ${numeric ? "ds-numeric" : ""}`}
      >
        {value}
      </p>
    </div>
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0 flex-1 space-y-1">
        ...
        {description ? (
          <p className="pl-[17px] text-xs text-on-surface-variant">
            {description}
          </p>
        ) : null}
      </div>
...
export function JobWorkspaceClient({
  ...
  const additionalDataStageLocked =
    job.stage === "FILING" || job.stage === "FILED";
  ...
  useEffect(() => {
    setIsAdditionalDataEditing(
      !Boolean(job.additionalData?.updatedAt) &&
        job.stage !== "FILING" &&
        job.stage !== "FILED",
    );
  }, [job.id]);
  ...
  const nodeNameMap = useMemo(() => {
    return new Map(
      Array.from(targetNodesMap.entries()).map(([key, node]) => [
        key,
        node?.name || key,
      ]),
    );
  ...
  const completedNodeKeys = useMemo(() => {
    if (!filingInstance?.nodeRuns) return new Set<string>();
    return new Set<string>(
      filingInstance.nodeRuns
        .filter((run: any) => run.status === "COMPLETED")
        .map((run: any) => run.nodeKey),
    );
  }, [filingInstance?.nodeRuns]);
  const nextStageOptions = useMemo<NextStageOption[]>(() => {
    return outgoingEdges.map((edge: any) => {
      const targetNode = targetNodesMap.get(edge.targetKey);
      const prerequisiteStatus = targetNode
  ...
  }, [completedNodeKeys, nodeNameMap, outgoingEdges, targetNodesMap]);
  const availableNextStageOptions = useMemo(
    () =>
      nextStageOptions.filter((option) => !option.prerequisiteStatus.isBlocked),
    [nextStageOptions],
  );
  ...
  const activeNodeDocumentRequirements = useMemo(() => {
    ...
    const rawPhotos =
      ...
      activeWorkflowVersionNode?.photoRequirements ??
      [];
    const rawChecklistItems =
      activeNodeRun?.node?.checklistItems ??
      activeWorkflowVersionNode?.checklistItems ??
      [];
    return normalizeUnifiedNodeDocuments(
      rawDocuments,
      rawPhotos,
      rawChecklistItems,
    );
  ...
  const primaryQuerySummary =
    activeNodeOpenQueries[0] || activeNodeQueries[0] || null;
  ...
  const filingCompletionColumnClass =
    "w-full space-y-4 xl:col-start-2 xl:self-start";
  ...
  const additionalDataLocked =
    additionalDataStageLocked ||
    (hasSavedAdditionalData && !isAdditionalDataEditing);
  ...
  const getChecklistVersionStatus = (versionId?: string | null) => {
    ...
    if (
      versionApprovals.some((approval: any) => approval.action === "APPROVED")
    ) {
  ...
  const handleUploadDoc = async (
    ...
    if (requiresValidity) {
      const prompted = window.prompt(
        "Enter validity date in YYYY-MM-DD format for this document.",
        "",
      );
      if (!prompted) {
        e.target.value = "";
      ...
      if (instanceRes.ok) {
        ...
        if (activeRun) {
          ...
          setChecklistResponses((current) => {
            ...
            return mergeChecklistResponsesForActiveNode(
              initialResponses,
              current,
            );
          });
          ...
          setFilingToggleStateDetails((current) => {
            if (activeNodeRun?.id !== activeRun.id) {
              return toggleStateDetailsForNode;
            }
            return {
        ...
        setNodeRemarks((current) =>
          activeNodeRun?.id !== activeRun?.id ? activeRun?.remarks || "" : current,
        );
      } else {
        toast.error(
          instanceRes.error ||
  ...
  useEffect(() => {
    if (!selectedNextNodeKey) return;
    const stillAvailable = availableNextStageOptions.some(
      (option: NextStageOption) => option.edge.targetKey === selectedNextNodeKey,
    );
    if (!stillAvailable) {
      setSelectedNextNodeKey("");
    }
  }, [availableNextStageOptions, selectedNextNodeKey]);
  ...
  const handleCompleteFilingNode = async (e: React.FormEvent) => {
    ...
    if (outgoingEdges.length > 0 && availableNextStageOptions.length === 0) {
      const blockedTargets = nextStageOptions
        .filter((option: NextStageOption) => option.prerequisiteStatus.isBlocked)
        .map((option: NextStageOption) => {
          const targetLabel =
            option.targetNode?.name || option.edge.targetKey;
          const missingList = formatSummaryList(
            option.prerequisiteStatus.missingNodeNames,
          );
          return missingList
            ? `${targetLabel} is locked until ${missingList} ${option.prerequisiteStatus.missingNodeNames.length === 1 ? "is" : "are"} completed`
            : `${targetLabel} is locked by prerequisite rules`;
        });
      toast.error(
        blockedTargets[0] ||
          "The next filing stages are locked by prerequisite rules.",
      );
      return;
    }
    ...
    if (activeNodeRun.node.commentsRequired && !nodeRemarks.trim()) {
      toast.error(
        `Comments are mandatory to complete stage: ${activeNodeRun.node.name}.`,
      );
      return;
    }
  ...
  const handleUploadChecklistItemFile = async (
    ...
    if (requirement?.requiresValidity) {
      const prompted = window.prompt(
        "Enter validity date in YYYY-MM-DD format for this filing upload.",
        "",
      );
      if (!prompted) {
        e.target.value = "";
  ...
  const handleExpenseReview = async () => {
    ...
    if (
      (expReviewStatus === "CLARIFICATION_REQUIRED" ||
        expReviewStatus === "REJECTED") &&
      !expReviewRemarks.trim()
    ) {
      toast.error(
  ...
  const stageProgress =
    activeStepIndex >= 0
      ? Math.round(((activeStepIndex + 1) / STAGES.length) * 100)
      : 0;
  ...
  return (
    <main className="w-full space-y-3 overflow-x-hidden pb-4">
        ...
        <div className="flex flex-col gap-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0 space-y-2">
            ...
            <div className="flex flex-wrap items-end gap-x-3 gap-y-1">
              <h1 className="ds-h1 ds-numeric text-on-surface">
                {job.jobNumber}
              </h1>
              <p className="max-w-4xl truncate text-sm font-medium text-on-surface">
                {job.title}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-on-surface-variant">
              <span>
                Customer:{" "}
                <span className="text-on-surface">{job.customer.name}</span>
              </span>
              <span className="text-outline">•</span>
              <span>
                Owner:{" "}
                <span className="text-on-surface">{job.primaryOwner.name}</span>
              </span>
              <span className="text-outline">•</span>
              <span>
                Manager:{" "}
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2 sm:flex sm:items-center">
            ...
            <div className="rounded-xl border border-outline-variant bg-surface-container-low px-3 py-2">
              ...
              <span className="mt-0.5 block text-xs text-[#00cec4] ds-numeric">
                {stageProgress}%
              </span>
            </div>
            {canDeleteJob ? (
            ...
            {STAGES.map((stage, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`whitespace-nowrap text-[10px] uppercase tracking-wide ${isActive ? "text-[#00cec4]" : "text-on-surface-variant"}`}
                  >
                    {stage.label}
                  </span>
                  {index < STAGES.length - 1 ? (
                    <span className="h-px w-5 bg-outline-variant/50" />
                  ) : null}
                </div>
              );
            })}
        ...
        <div className="rounded-2xl border border-[#fb923c]/35 bg-[#fb923c]/10 p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="space-y-1">
              <span className="ds-label text-[#fb923c]">
                Job Settings Alert
              </span>
      ...
      <nav className="sticky top-0 z-20 w-full rounded-xl border border-outline-variant/25 bg-surface/95 px-4 py-2 backdrop-blur supports-[backdrop-filter]:bg-surface/85">
        <div className="flex w-full flex-wrap items-stretch justify-between gap-x-2 gap-y-1">
          {workspaceTabs.map((tab) => {
            const isActive = activeTab === tab.key;
            return (
              <div
                key={tab.key}
                className="flex min-w-[140px] flex-1 basis-[150px]"
              >
                <button
                  type="button"
                  ...
                  className="ds-plain cha-link cha-workspace-tab"
                >
                  <span>{tab.label}</span>
                  {tab.count !== undefined ? (
                    <span className="ds-numeric shrink-0">({tab.count})</span>
                  ) : null}
                </button>
              </div>
            );
      ...
      <div className="min-h-[320px] rounded-xl border border-outline-variant/60 bg-surface p-3 shadow-sm sm:p-4">
            ...
            <div className="flex items-start justify-between gap-4 border-b border-outline-variant/20 pb-4">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                <Button
                  ...
                  disabled={
                    loading !== null || bulkNaEligibleRequirements.length === 0
                  }
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-outline-variant bg-surface-container-low p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-on-surface-variant">
                  Activate Section 49 here to reveal its document card in the
                  list below.
                </p>
              </div>
              <Button
              ...
              if (categoryKeys.length === 0) {
                return (
                  <p className="text-sm text-on-surface-variant italic py-4">
                    No document requirements configured for this job.
                  </p>
                );
              }
                              ...
                              <div className="grid items-start grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                                {section.requirements.map((req: any) => {
                                  ...
                                  const isPendingRequirement =
                                    section.key === "pending";
                                  const currentVersion = req.versions.find(
                                    (v: any) => v.isCurrent,
                                  );
                                  const isSection49Requirement =
                                    req.name === "Section 49";
                                  const effectiveValidityDate =
                                    isSection49Requirement
                                      ? section49Flag?.validityDate ||
                                        currentVersion?.validityDate
                                      : currentVersion?.validityDate;
                                  const validitySummary = getValiditySummary(
                                    effectiveValidityDate || null,
                                  );
                                  const section49WarningActive =
                                    isSection49Requirement &&
                                    effectiveValidityDate &&
                                    ["warning", "destructive"].includes(
                                      validitySummary?.tone || "",
                                    );
                                  return (
                                    <div
                                      key={req.id}
                                          ...
                                          <div className="mt-3 space-y-3 rounded-xl border border-outline-variant/40 dark:border-[#00cec4]/20 bg-surface-container-low p-3.5">
                                            {section49Flag?.validityDate ? (
                                              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant/20 pb-2.5">
                                                <div className="flex items-center gap-2">
                                                  <span className="ds-label text-on-surface-variant font-medium">
                                            ...
                                            {section49Flag?.validityDate ? (
                                              <div className="space-y-3 pt-1">
                                                ...
                                                <p className="text-[11px] leading-relaxed text-on-surface-variant">
                                                  {!section49WarningActive ? (
                                                    <span className="text-[#fb923c] font-medium">
                                                      ⚠️ Extension entry is
                                                      saved here, but submission
                                                      becomes available only
                                                      within 4 days of expiry.
                                                    </span>
                                                  ) : (
                                                    <span>
                                                      An extension document and
                                                      new validity date are
                                                ...
                                                job.section49Extensions.length >
                                                  0 ? (
                                                  <div className="space-y-1 border-t border-outline-variant/30 pt-3 text-xs">
                                                    ...
                                                    {job.section49Extensions
                                                      ...
                                                      .map((extension: any) => (
                                                        <div
                                                          key={extension.id}
                                                          className="flex flex-wrap items-center justify-between gap-2 text-on-surface-variant"
                                                        >
                                          ...
                                          <div className="mt-3 rounded-lg border border-outline-variant/40 bg-surface p-2.5 text-xs">
                                            <div className="flex items-center justify-between gap-3">
                                              <div className="flex min-w-0 items-center gap-2 truncate">
                                                ...
                                                />
                                                {currentVersion.fileKey.startsWith(
                                                  "http",
                                                ) ? (
                                                  <a
                                                    href={
                                                      currentVersion.fileKey
                                                    }
                                                    target="_blank"
              ...
              <div className="pt-4 border-t border-outline-variant/30 flex flex-col items-end gap-3">
                {proceedErrors && (
                  <div className="w-full md:max-w-xl p-4 rounded-2xl border border-orange-500/30 bg-orange-500/10 text-orange-500 text-xs">
                    <p className="font-semibold uppercase tracking-wider ds-label text-orange-500 mb-1">
                      Proceed Blocked
                    </p>
                    <p>{proceedErrors[0]}</p>
                  </div>
          ...
          <div className="space-y-4">
            ...
            {job.stage === "DOCUMENT_COLLECTION" ? (
              <div className="flex items-start gap-3 rounded-2xl border border-[#fb923c]/40 bg-surface p-4">
                ...
                />
                <div>
                  <h4 className="text-sm font-bold uppercase tracking-wide text-[#fb923c]">
                    DOCUMENT GATE REQUIRED
                  </h4>
                  <p className="mt-1 text-xs leading-relaxed text-on-surface-variant">
                    Complete Document Collection before saving or completing
                    Additional Data.
                  </p>
                </div>
              </div>
            ...
            {manifestConfigMissing ? (
              <div className="flex items-start gap-3 rounded-2xl border border-[#fb923c]/40 bg-surface p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-on-surface-variant">
                    This clearance type is missing manifest configuration.
                    Please update it in CHA settings before continuing.
                  </p>
                </div>
              </div>
            ) : null}
              ...
              <AdditionalDataStatCard
                ...
                value={
                  job.additionalData?.updatedAt
                    ? new Date(job.additionalData.updatedAt).toLocaleDateString(
                        "en-IN",
                      )
                    : "Not saved"
                }
                  ...
                  {job.additionalData ? (
                    <div className="mt-4 pt-4">
                      <DoValidityPanel
                        ...
                        additionalData={{
                          deliveryOrderValidity:
                            job.additionalData.deliveryOrderValidity ?? null,
                          deliveryOrderExtensionDate:
                            job.additionalData.deliveryOrderExtensionDate ??
                            null,
                          doDocumentFileKey:
                            job.additionalData.doDocumentFileKey ?? null,
                          doDocumentFileName:
                            job.additionalData.doDocumentFileName ?? null,
                          doDocumentUploadedAt:
                            job.additionalData.doDocumentUploadedAt ?? null,
                        }}
            ...
            <div className="flex flex-col gap-2 pt-4 sm:flex-row sm:justify-end">
              {hasSavedAdditionalData && !additionalDataStageLocked ? (
                <Button
                  ...
                  variant="outline"
                  disabled={
                    loading !== null || isAdditionalDataEditing || !canUpdateJob
                  }
          ...
          <div className="space-y-4">
            <SectionHeading
              ...
              aside={
                <div className="flex flex-wrap items-center gap-2">
                  <Badge
                    variant={currentChecklistVersion ? "default" : "warning"}
                    className="uppercase"
                  >
                    {checklistWorkflow?.status?.replace(/_/g, " ") ||
                      "Pending Upload"}
                  </Badge>
            ...
            {activeStepIndex < checklistStageIndex ? (
              <div className="bg-surface border border-[#fb923c]/40 p-4 rounded-2xl flex items-start gap-3">
                ...
                />
                <div>
                  <h4 className="font-bold text-sm text-[#fb923c]">
                    CHECKLIST PREPARATION NOT AVAILABLE
                  </h4>
              ...
              <div className="space-y-4">
                {!job.assignedManagerId && (
                  <div className="bg-surface border border-[#fb923c]/40 p-4 rounded-2xl flex items-start gap-3">
                    ...
                    />
                    <div>
                      <h4 className="font-bold text-sm uppercase text-[#fb923c]">
                        Manager Assignment Recommended
                      </h4>
                      <p className="text-xs text-on-surface-variant mt-1 leading-relaxed">
                        No manager has been assigned to this job yet. Internal
                        approval can still be completed by the job owner, or
                        route through the owner&apos;s Manager or TL if
                        configured in HRMS, but assigning a manager keeps
                    ...
                    <div className="space-y-4">
                      ...
                      {internalApproversCount === 0 && (
                        <div className="bg-surface border border-red-500/40 p-4 rounded-2xl flex items-start gap-3">
                          ...
                          />
                          <div>
                            <h4 className="font-bold text-xs uppercase text-red-500">
                              No Internal Approvers Configured
                            </h4>
                            <p className="text-xs text-on-surface-variant mt-1 leading-relaxed">
                              There are no active internal checklist approvers
                              configured for this job. Please assign an employee
                              with the{" "}
                              <span className="font-medium text-on-surface">
                                Approval
                              </span>{" "}
                              responsibility in the job settings, or verify that
                              the job owner, assigned manager, or owner&apos;s
                              team lead is configured in HRMS.
                            </p>
                            {canManageSettings && (
                              <button
                                type="button"
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                            <p className="max-w-xl text-xs text-on-surface-variant">
                              ...
                              be fetched from the customer record, and any files
                              above will be included after you send the mail.
                            </p>
                            <Button
                              type="button"
                              disabled={loading !== null}
                                    ...
                                    <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                                      <div className="space-y-1">
                                        ...
                                        <p className="text-xs text-on-surface-variant">
                                          Assigned to{" "}
                                          <span className="text-on-surface">
                                            {getUserName(approval.assignedToId)}
                                          </span>
                                          {approval.actedById
            ...
            {overdueChecklistCount > 0 && (
              <div className="card-left-accent-orange rounded-xl border border-outline-variant/60 bg-surface p-4 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  />
                  <div className="space-y-1">
                    <h4 className="ds-label text-[#fb923c]">
                      Overdue Filing Checklist Items
                    </h4>
                    <p className="text-sm text-on-surface">
                      {overdueChecklistCount} Filing checklist item
                      {overdueChecklistCount > 1 ? "s are" : " is"} overdue.
                    </p>
                    <p className="text-xs text-on-surface-variant">
                      Delay remarks are required before overdue items can be
                      completed.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-on-surface-variant leading-relaxed">
                      Clearance files can only be submitted to customs after the
                      checklist is approved. Complete all prior checklist
                    ...
                    {loading === "filing-load" ? (
                      ...
                      <div className="mt-4 space-y-4">
                        <p className="max-w-3xl text-sm text-on-surface-variant leading-relaxed">
                          No active filing workflow instance found. Ensure a
                          workflow is published in{" "}
                          <a
                            href="/cha/settings/filing-workflows"
                            className="text-[#00cec4] underline underline-offset-2"
                          >
                  ...
                  <div className="space-y-4">
                    {activeNodeRun ? (
                      <div className="space-y-4">
                        ...
                        >
                          <div
                            className={`card-top-accent ${filingPrimaryColumnClass} space-y-3 rounded-xl border border-outline-variant/60 bg-surface p-4 shadow-sm`}
                          >
                            <SectionHeading
                            ...
                            <div className="flex flex-wrap gap-2 pl-[17px]">
                              {activeNodeRun.node.nodeType === "DECISION" ? (
                                <Badge variant="default">Decision</Badge>
                              ) : null}
                            ...
                            {overdueChecklistCount > 0 && (
                              <p className="pl-[17px] text-xs text-[#fb923c]">
                                {overdueChecklistCount} overdue checklist item
                                {overdueChecklistCount > 1 ? "s" : ""} in this
                                active stage.
                              </p>
                            )}
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {(
                                    ...
                                    <Button
                                      ...
                                      disabled={loading !== null}
                                      onClick={() =>
                                        void handleRedirectBlockedStage(nodeKey)
                                      }
                                    >
                                      <ArrowRight size={14} />
                                      Go To{" "}
                                      {targetNodesMap.get(nodeKey)?.name ||
                                        nodeKey}
                                    </Button>
                                  ))}
                                </div>
                            ...
                            canResumePendingBlockedStage ? (
                              <div className="rounded-xl border border-[#00cec4]/35 bg-surface-container-low px-4 py-3">
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                  <div className="space-y-1">
                                    <p className="ds-label text-[#00cec4]">
                                      Blocked Stage Ready
                                    </p>
                                    <p className="text-sm text-on-surface">
                                      {pendingBlockedStage.nodeName} can now be
                                      resumed.
                                    </p>
                                  </div>
                                  <Button
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                                    {outgoingEdges.map((edge: any) => {
                                      ...
                                      const isSelected =
                                        selectedNextNodeKey === edge.targetKey;
                                      return (
                                        <button
                                          key={edge.targetKey}
                                          type="button"
                                    ...
                                    <div className="space-y-2.5">
                                      {activeNodeDocumentRequirements.map(
                                        (requirement: any) => {
                                          ...
                                          const minUploads = Number(
                                            requirement.minUploads ?? 1,
                                          );
                                          const maxUploads =
                                          ...
                                          return (
                                            ...
                                            >
                                              <div className="flex items-start justify-between gap-3">
                                                <div>
                                                  <p className="ds-label text-on-surface">
                                                    {requirement.label}
                                                  </p>
                                                ...
                                                <span className="ds-label text-on-surface">
                                                  MIN {minUploads}
                                                  {maxUploads
                                                    ? ` / MAX ${maxUploads}`
                                                    : ""}
                                              ...
                                              <FileUploadField
                                                ...
                                                helperText={`Accepted formats: PDF and images. Uploading here ${requirement.allowReplacement !== false ? "replaces the current" : "adds the"} ${requirement.label}.${requirement.requiresValidity ? " Capture validity metadata if required." : ""}`}
                                                  ...
                                                  uploadedAttachment
                                                    ? {
                                                        ...
                                                        name:
                                                          ...
                                                          `${requirement.label}.pdf`,
                                                        sizeBytes:
                                                          uploadedAttachment.sizeBytes ??
                                                          null,
                                                        statusLabel:
                                                          uploadedAttachment.uploadedAt
                              ...
                              {activeNodeRun.node.checklistItems?.length >
                                0 && (
                                <div className="space-y-3 pt-1">
                                  <h4 className="ds-label text-on-surface">
                                        ...
                                        const isCompletedItem =
                                          ...
                                          (!(
                                            ...
                                            checklistItemAttachments.length >=
                                              item.minUploads);
                                        const checklistUploadHeading =
                                          item.documentType?.trim() ||
                                          "Supporting File / Photo";
                                        const isLockedItem =
                                          index > currentChecklistItemIndex;
                                        return (
                                          <div
                                          ...
                                          >
                                            <div className="flex items-center justify-between gap-4">
                                              <div className="flex min-w-0 flex-1 items-center gap-3">
                                                ...
                                                <div className="min-w-0 flex-1 space-y-1">
                                                  <label
                                                    ...
                                                    className="block cursor-pointer text-sm font-semibold leading-5 text-on-surface"
                                                  >
                                                    {item.label}{" "}
                                                    {item.isMandatory && (
                                              ...
                                              <div className="shrink-0 self-center text-right">
                                                <div className="ds-label text-on-surface-variant">
                                                  Item {index + 1} of{" "}
                                            ...
                                            {isLockedItem && (
                                              <div className="rounded-xl border border-outline-variant/60 bg-surface px-3 py-2 text-[11px] text-on-surface-variant">
                                                Complete the current checklist
                                                item first to unlock this step.
                                              </div>
                                            )}
                                            ...
                                            {!isLockedItem &&
                                              ...
                                              isCompletedItem && (
                                                <div className="rounded-xl border border-outline-variant/60 bg-surface px-3 py-2 text-[11px] text-on-surface-variant">
                                                  Completed and ready. You can
                                                  move to the next checklist
                                                  item.
                                                </div>
                                              )}
                                            ...
                                            {!isLockedItem && overdueMeta && (
                                              <div className="rounded-2xl border border-[#fb923c]/35 bg-surface px-3 py-2 text-xs text-on-surface">
                                                <div className="flex flex-wrap items-center gap-3">
                                                  <span className="font-semibold text-[#fb923c] uppercase tracking-wide">
                                                    Overdue
                                                  </span>
                                                  <span className="ds-numeric">
                                                    Due:{" "}
                                              ...
                                              activeNodeDocumentRequirements.length === 0 && (
                                                <div className="pl-6 space-y-2">
                                                  ...
                                                  <FileUploadField
                                                    ...
                                                    triggerText={`Drag and drop or choose ${checklistUploadHeading.toLowerCase()} to upload`}
                                                    ...
                                                    <div className="space-y-1">
                                                      {checklistItemAttachments.map(
                                                        (attachment: any) => (
                                                          <div
                                                            key={attachment.id}
                                                            className="flex items-center justify-between rounded-lg bg-surface px-2 py-1 text-xs"
                                                          >
                                                            <a
                                                              href={
                                                                attachment.fileKey
                              ...
                              <div className="space-y-2.5">
                                {visibleNodeConditionalSections.map(
                                  (section: any) => (
                                    <div
                                      key={section.key}
                                      className="rounded-xl border border-outline-variant bg-surface-container-low p-3 space-y-2.5"
                                    >
                                      <div className="flex items-center gap-3 text-sm text-on-surface">
                                        <NeonCheckbox
                                          checked={
                                            !!filingToggleStates[section.key]
                                          }
                                          onChange={(e) => {
                                            const checked = e.target.checked;
                                            setFilingToggleStates((prev) => ({
                                              ...prev,
                                              [section.key]: checked,
                                            }));
                                            setFilingToggleStateDetails(
                                      ...
                                      {filingToggleStates[section.key] &&
                                        section.unlocksFields?.length > 0 && (
                                          <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2">
                                      ...
                                      {filingToggleStates[section.key] &&
                                        section.unlocksDocuments?.length >
                                          0 && (
                                          <div className="space-y-1.5">
                                            {section.unlocksDocuments.map(
                                              (requirement: any) => (
                                                <div
                                                  key={requirement.key}
                                                  className="rounded-xl border border-outline-variant/60 bg-surface p-3"
                                                >
                                                  <div className="flex items-center justify-between gap-3">
                                                    <div>
                                ...
                                <div className="card-top-accent-orange overflow-hidden rounded-xl border border-outline-variant/60 bg-surface shadow-sm">
                                  <div className="flex flex-col gap-3 px-4 py-3 lg:flex-row lg:items-start">
                                    <div className="min-w-0 flex-1">
                                      <div className="space-y-2">
                                        <div className="grid grid-cols-[4px_minmax(0,1fr)] items-center gap-3">
                                          <span
                                            className="h-7 rounded-sm bg-[#fb923c]"
                                            aria-hidden="true"
                                          />
                                          <h3 className="ds-h3 text-[#c76628]">
                                            Query Processing
                                          </h3>
                                        </div>
                                    ...
                                    <div className="flex shrink-0 items-center justify-end gap-3 self-start lg:ml-auto">
                                      ...
                                      <label className="inline-flex min-w-[220px] items-center justify-between gap-4 rounded-xl border border-[#fb923c]/20 bg-[#fb923c]/8 px-4 py-2 text-sm text-on-surface">
                                        <span className="ds-label text-[#c76628]">
                                                ...
                                                !queryProcessingToggleEnabled
                                                  ? {
                                                      ...(queryProcessingState ??
                                                        {}),
                                                      stage:
                                                        typeof queryProcessingState?.stage ===
                                                          "string" &&
                                                        queryProcessingState.stage.trim()
                                  ...
                                  {queryProcessingActive ? (
                                    ...
                                    <div className="px-4 py-3 text-sm text-on-surface-variant">
                                      Turn on queries to reveal the customs
                                      query card, summary details, and manage
                                      actions for this filing step.
                                    </div>
                                  )}
                                </div>
                                    ...
                                    <div className="space-y-5">
                                      ...
                                      queryProcessingStage === "CLEARED" ? (
                                        <div className="rounded-xl border border-[#00cec4]/25 bg-[#00cec4]/10 p-4 text-sm text-on-surface">
                                          All customs queries are cleared for
                                          this workflow stage. You can continue
                                          to the next stage or record an
                                          additional query if customs raises
                                          another one.
                                          <div className="mt-3">
                                            <Button
                                              type="button"
                                      ...
                                      <div className="space-y-4 ds-form-section">
                                        <div className="flex items-start justify-between gap-3">
                                          <div>
                                            ...
                                            <p className="pl-[13px] text-sm text-on-surface-variant">
                                              ...
                                              reference number, officer, and
                                              received date.
                                            </p>
                                          </div>
                                          <Badge variant="default">
                                            {activeNodeQueries.length} Case
                                            {activeNodeQueries.length === 1
                                              ? ""
                                              : "s"}
                                          </Badge>
                                        </div>
                                        ...
                                        <div className="flex justify-end">
                                          <Button
                                            type="button"
                                            onClick={handleCreateFilingQuery}
                                            disabled={loading !== null}
                                          >
                                            {loading === "filing-query-create"
                                              ? "Saving..."
                                              : "Create Query Record"}
                                          </Button>
                                        </div>
                                      ...
                                      <div className="space-y-3 ds-form-section">
                                        <div className="flex items-center justify-between gap-3">
                                          <div>
                                            <h4>Query Cases</h4>
                                            <p className="pl-[13px] text-sm text-on-surface-variant">
                                              View responses, respond offline,
                                              submit updates, clear, or reopen
                                              any customs query.
                                            </p>
                                          </div>
                                          <Badge variant="default">
                                ...
                                <div className="card-top-accent overflow-hidden rounded-xl border border-outline-variant/60 bg-surface shadow-sm">
                                  <div className="space-y-4 px-4 py-4">
                                    ...
                                    <div className="rounded-xl border border-outline-variant/60 bg-surface-container-low px-4 py-3">
                                      ...
                                      {(activeNodeRun.node.sectionName ||
                                        activeNodeRun.node.branchName) && (
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <textarea
                                        ...
                                        required={
                                          activeNodeRun.node.commentsRequired
                                        }
                                      />
                                    </div>
                                    <div className="space-y-2">
                                      {availableNextStageOptions.length > 0 ? (
                                        activeNodeRun.node.nodeType ===
                                        "DECISION" ? null : (
                                          <div className="space-y-1">
                                            <label className="ds-label text-on-surface block">
                                              Select Next Workflow Stage *
                                            </label>
                                            <select
                                              ...
                                              {availableNextStageOptions.map(
                                                (option: any) => {
                                                  ...
                                                  return (
                                                    ...
                                                    >
                                                      {[
                                                        targetNode?.sectionName,
                                                        targetNode?.branchName,
                                                        targetNode?.name ||
                                                          option.edge.targetKey,
                                                      ]
                                                        .filter(Boolean)
                                        ...
                                        <div className="rounded-xl border border-[#fb923c]/35 bg-surface px-3 py-2 text-xs text-on-surface">
                                          Next stage is locked by prerequisites.
                                          Complete the required earlier stage
                                          first.
                                        </div>
                                      ) : (
                                        <div className="rounded-xl border border-outline-variant bg-surface-container-low p-3 text-xs text-on-surface-variant">
                                          Completing this node will finalize the
                                          Filing workflow and transition the job
                                          stage to <strong>FILED</strong>.
                                        </div>
                                      )}
                                    </div>
                                    <div className="flex flex-wrap justify-end gap-2 pt-1">
                                      ...
                                      <Button
                                        ...
                                        disabled={
                                          ...
                                          isActiveStageBlocked ||
                                          (outgoingEdges.length > 0 &&
                                            availableNextStageOptions.length ===
                                              0)
                                        }
                          ...
                          <div className="space-y-4">
                            <label className="block space-y-1.5">
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  setSelectedJumpBackNodeKey(e.target.value)
                                }
                                className="w-full text-sm"
                              >
                                <option value="">
                                  -- Choose Earlier Stage --
                                </option>
                                {jumpBackTargets.map((target: any) => (
                                  ...
                                  >
                                    {target.nodeName}{" "}
                                    {target.completedAt
                                      ? `(${new Date(target.completedAt).toLocaleString("en-IN")})`
                                      : ""}
                                  </option>
                                ))}
                              </select>
                      ...
                      <div className="card-top-accent rounded-xl border border-outline-variant/60 bg-surface p-5 space-y-4 shadow-sm">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <div className="space-y-1">
                            <h4 className="ds-h3 text-on-surface">
                        ...
                        <p className="max-w-xl text-sm text-on-surface-variant">
                          All blueprint checklist checks have been completed and
                          the customs submission has been filed. The job stage
                          is updated to <strong>FILED</strong>.
                        </p>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="space-y-4">
                <div className="border border-outline-variant p-4 rounded-2xl space-y-4">
                  <span className="ds-label block text-on-surface">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setExpectedAdvance(parseFloat(e.target.value) || 0)
                      }
                ...
                {showWaiveAdvance && (
                  <div className="border border-red-200 bg-red-50/5 p-4 rounded-2xl space-y-3">
                    <span className="ds-label text-red-500 block">
                ...
                {job.customerAdvance.status !== "NOT_REQUIRED" &&
                  job.customerAdvance.status !== "FULLY_RECEIVED" && (
                    <form
                      ...
                      className="border border-outline-variant p-4 rounded-2xl space-y-4"
                    >
                      <span className="ds-label block text-on-surface">
                  ...
                  {job.customerAdvance.receipts?.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {job.customerAdvance.receipts.map((r: any) => (
                        <div
                          key={r.id}
                          className="p-3 bg-surface-container-low border border-outline-variant/40 rounded-2xl flex items-center justify-between text-xs"
                        >
                          <div>
                            <span className="font-bold text-[#00cec4] block">
        ...
        {activeTab === "expenses" && (
          <div className="space-y-4">
                      ...
                      label={
                        <div>
                          ...
                          <span className="text-xs text-on-surface-variant">
                            Request accounts to disburse payment immediately to
                            resolve critical port blocks.
                          </span>
                        </div>
                      }
                    />
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-3">
                    {expenseLines.map((line, index) => (
                      <div
                        key={index}
                        className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end border-b border-outline-variant/20 pb-3 md:pb-0 md:border-b-0"
                      >
                        {/* category */}
            ...
            <div className="space-y-4">
              <h3 className="ds-h3 text-on-surface border-b border-outline-variant/30 pb-2">
                Expenses Queue
              </h3>
                      ...
                      >
                        ...
                        <div className="space-y-1.5 pl-2 border-l border-outline-variant/40">
                          {req.lines.map((l: any) => (
                            ...
                            >
                              <span className="text-on-surface">
                                <strong className="text-on-surface-variant uppercase">
                                  {l.category}
                                </strong>
                                : {l.purpose}
                              </span>
                              <span className="font-mono ds-numeric text-on-surface-variant">
                        ...
                        {req.queries?.map((q: any) => (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-on-surface italic mt-0.5">
                                "{q.queryText}"
                              </p>
                            ...
                            {q.resolved ? (
                              <div className="border-t border-orange-200/50 pt-2 text-[11px] text-green-700">
                                <strong>Resolution:</strong> "{q.resolutionText}
                                " (Resolved by {q.resolvedBy?.name})
                              </div>
                            ) : resolveQueryId === q.id ? (
                              <div className="space-y-2 border-t border-orange-200/50 pt-2">
                                ...
                                <div className="flex justify-end gap-1">
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => setResolveQueryId(null)}
                                  >
                                    Cancel
                                  </Button>
                                  <Button
                        ...
                        {req.payments?.map((p: any) => (
                          <div
                            key={p.id}
                            className="p-3 bg-green-50/5 border border-green-200 rounded-lg text-xs flex flex-col md:flex-row md:items-center justify-between gap-2"
                          >
                            <div>
                              <span className="font-semibold text-green-700 block">
                                Payment Disbursed via {p.paymentMethod}
                              </span>
                              <span className="text-[10px] text-on-surface-variant">
                                Txn Ref: {p.transactionReference} • Paid by{" "}
                                {p.paidBy?.name}
                              </span>
                            </div>
                        ...
                        {escRequestId === req.id && (
                          <div className="p-4 border border-[#fb923c]/40 bg-[#fb923c]/5 rounded-2xl space-y-3">
                            <span className="ds-label text-[#fb923c] font-bold block">
                          ...
                          <div className="p-4 border border-outline-variant/60 bg-surface rounded-2xl space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <select
                                ...
                                onChange={(e) =>
                                  setExpReviewStatus(e.target.value)
                                }
                                className="text-xs w-full"
                              >
                                ...
                                <option value="READY_FOR_DISBURSEMENT">
                        ...
                        {payRequestId === req.id && (
                          <form
                            ...
                            className="p-4 border border-[#00cec4]/40 bg-[#00cec4]/5 rounded-2xl space-y-4"
                          >
                            <span className="ds-label text-[#00cec4] block">
                          ...
                          <div className="p-4 border border-[#fb923c]/40 bg-[#fb923c]/5 rounded-2xl space-y-3">
                            ...
                            <div className="flex justify-end gap-2">
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setQueryRequestId(null)}
                              >
                                Cancel
                              </Button>
                              <Button
                        ...
                        <div className="flex justify-end gap-2 border-t border-outline-variant/10 pt-3 text-xs">
                          ...
                          {/* Accounts Payout */}
                          {(req.status === "APPROVED" ||
                            req.status === "READY_FOR_DISBURSEMENT" ||
                            req.isUrgent) &&
                            req.status !== "PAID" &&
                            req.status !== "RECEIPT_ACKNOWLEDGED" &&
                            req.status !== "REJECTED" && (
                ...
                {job.auditLogs.map((log: any) => (
                  <div key={log.id} className="relative space-y-1 text-xs">
                    ...
                    <div className="flex gap-4 text-[10px] text-on-surface-variant pt-0.5">
                      <span>
                        Actor: <strong>{log.actor?.name || "System"}</strong>
                      </span>
                      {log.newState && (
                        <span>
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h4
                          className={`text-sm font-medium ${isCurrent ? "text-[#00cec4]" : "text-on-surface"}`}
                        >
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-red-200/70 bg-red-50/40 p-4 text-sm text-on-surface">
            ...
            <p className="mt-1 text-on-surface-variant">
              Deleting this job affects linked CHA workflows, audit visibility,
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-on-surface-variant">
                Enter exactly:{" "}
                <span className="text-on-surface">delete job</span>
              </p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-red-200/70 bg-red-50/40 p-4 text-sm text-on-surface">
            ...
            <p className="mt-1 text-on-surface-variant">
              ...
              <span className="text-on-surface">delete job</span> to execute
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <Button
              variant="destructive"
              disabled={
                !deleteDecisionRemarks.trim() || loading === "job-delete-reject"
              }
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-xs text-red-500 font-medium">
              ...
              mandatory document requirement, the workflow stage may revert to
              Document Collection.
            </p>
            <div className="flex justify-end gap-2">
              <Button
        ...
        >
          <div className="space-y-4">
            <div className="rounded-xl border border-outline-variant bg-surface-container-low p-4 text-xs text-on-surface-variant">
              ...
              inside <strong className="text-on-surface">this CHA job</strong>,
              under the{" "}
              <strong className="text-on-surface">User Uploads</strong> section.
            </div>
                ...
                {canPreview ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-4 text-center space-y-4">
                    ...
                    <div className="space-y-2 max-w-md">
                      <h4 className="ds-h3 text-sm text-on-surface">
                        Preview Unavailable
                      </h4>
                    ...
                    <div className="p-4 rounded-xl border border-outline-variant bg-surface-container-low text-left text-xs space-y-2 w-full max-w-md">
                      <p className="font-semibold text-on-surface uppercase ds-label">
                        File Details
                      </p>
                      <div className="grid grid-cols-2 gap-2 text-on-surface-variant">
                        <span>Filename:</span>
                        <span className="text-on-surface truncate font-sans">
