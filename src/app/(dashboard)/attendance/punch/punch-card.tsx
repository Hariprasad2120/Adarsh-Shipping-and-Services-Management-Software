...
export function PunchCard({
  ...
  const [nowState, setNowState] = useState<Date>(() => {
    const initialDate = new Date(initialNowIso);
  ...
  const selectedDayOtMinutes = useMemo(() => {
    if (!selectedDayMetrics.dayOt || selectedDayMetrics.dayOt.otHours <= 0) {
      return 0;
    }
    return Math.round(selectedDayMetrics.dayOt.otHours * 60);
    ...
    <div className="grid gap-6 lg:grid-cols-4">
      ...
      <div className="flex flex-col gap-6 lg:col-span-3 lg:max-h-[calc(100vh-180px)]">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {canManage && employees.length > 0 && (
              <div className="w-64">
                <DropdownSelect
                  ...
                  triggerClassName="w-full border-mono-border/60 bg-mono-card text-mono-text"
                />
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                size="sm"
                className="size-8 rounded-xl p-0 text-mono-muted hover:bg-mono-card"
                disabled={isPending}
        ...
        >
          ...
          <div className="shrink-0 grid grid-cols-[88px_minmax(0,1fr)_72px_112px] items-center gap-4 border-b border-mono-border/30 bg-mono-soft px-6 py-4 text-[10px] font-medium tracking-[0.08em] text-mono-muted select-none">
            <div>Day</div>
            <div className="relative flex justify-between px-2 text-[8px] tracking-[0.06em] text-mono-muted overflow-visible">
              <span>9:30 AM</span>
              <span>10:30 AM</span>
              <span>11:30 AM</span>
              <span>12:30 PM</span>
            ...
            {daysInMonth.map((day) => {
              ...
              if (firstIn) {
                ...
                visualBar = (
                  <div
                    ...
                    className="relative flex h-6 w-full items-center overflow-visible"
                  >
                    {/* Center axis line */}
                ...
                if (day.dayOfWeek === 6) {
                  visualBar = (
                    <div
                      ref={isCurrentDay ? currentDayTrackRef : undefined}
                      className="relative flex h-6 w-full items-center justify-center rounded-full border border-secondary/10 bg-secondary/5 select-none overflow-visible"
                    >
                  ...
                  visualBar = (
                    <div
                      ref={isCurrentDay ? currentDayTrackRef : undefined}
                      className="relative flex h-6 w-full items-center justify-center rounded-full border border-dashed border-mono-border/80 bg-mono-soft select-none overflow-visible"
                    >
                ...
                visualBar = (
                  <div
                    ref={isCurrentDay ? currentDayTrackRef : undefined}
                    className="relative flex h-6 w-full items-center select-none overflow-visible"
                  >
              ...
              return (
                <div
                  ...
                  className={cn(
                    "grid cursor-pointer grid-cols-[88px_minmax(0,1fr)_72px_112px] items-center gap-4 border-l-2 px-6 py-3 transition select-none",
                    isSelected
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="whitespace-nowrap font-mono text-xs font-medium tabular-nums text-mono-text">
                      {hoursWorked > 0
        ...
        <div className="mnx-content-wide flex h-full flex-col border border-mono-border/40 bg-mono-card shadow-sm">
          ...
          <div className="space-y-2.5 border-b border-mono-border/30 px-5 py-4 sm:px-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex min-w-0 items-center gap-2.5">
                ...
                <h3 className="text-sm font-medium leading-none tracking-[0.01em] text-mono-text">
                  {selectedDateFormatted}
                </h3>
              </div>
              <span className="inline-flex items-center gap-1.5 self-center text-[10px] font-medium uppercase tracking-[0.12em] text-mono-muted">
                <Sparkles className="size-3.5 text-[var(--mnx-accent)]" />
                Day pulse
              </span>
            </div>
            <p className="text-xs font-medium tracking-[0.01em] text-mono-text/85">
          ...
          <div className="flex-1 space-y-5 overflow-y-auto px-5 py-4 sm:px-6">
            ...
            <div className="space-y-4 rounded-[1.6rem] border border-mono-border/35 bg-mono-soft/70 p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <span className="text-xs font-medium tracking-[0.04em] text-mono-text">
                  Attendance Status
                </span>
                {selectedDayMetrics.dayPunch?.inAt ? (
                  <span
                    className={`inline-flex items-center gap-2 rounded-full border border-[var(--mnx-accent)]/20 bg-[var(--mnx-accent)]/10 px-3 py-1 text-xs font-medium text-[var(--mnx-accent)] dark:text-[var(--mnx-accent)] ${!selectedDayMetrics.dayPunch?.outAt && selectedDateStr === todayStr ? "animate-pulse" : ""}`}
                  >
              ...
              <div className="overflow-hidden rounded-[1.25rem] border border-mono-border/20 bg-mono-card/70">
                <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-6 border-b border-mono-border/20 px-6 py-4">
                  <p className="text-[10px] font-medium uppercase tracking-[0.12em] text-mono-text">
                    First In
                  </p>
                  <p className="text-xs font-medium tabular-nums text-mono-text text-right">
                    {timelineSessions.length > 0
            ...
            <div className="space-y-4">
              <p className="text-xs font-medium tracking-[0.04em] text-mono-text">
                Punch Sessions
              </p>
              {timelineError ? (
              ...
              {timelineLoading ? (
                <div className="space-y-3">
                  <div className="h-24 animate-pulse rounded-2xl border border-mono-border/30 bg-mono-soft" />
                  <div className="h-24 animate-pulse rounded-2xl border border-mono-border/30 bg-mono-soft" />
                </div>
              ) : timelineSessions.length === 0 ? (
                <div className="select-none rounded-2xl border border-dashed border-mono-border/50 py-6 text-center text-xs font-medium text-mono-muted">
                  ...
                  {timelineSessions.map((session, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="grid gap-4 sm:grid-cols-2">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-1 pl-5.5">
                              <p className="text-xs font-medium leading-none text-mono-text">
                                {fmt(session.in)}
                              </p>
                              <p
                                className="break-words text-[10px] font-medium leading-5 text-mono-text/80"
                          ...
                          <div className="space-y-2 border-t border-mono-border/30 pt-4 sm:border-t-0 sm:border-l sm:pl-5 sm:pt-0">
                            ...
                            <div className="space-y-1 pl-5.5">
                              {session.out ? (
                                ...
                                <span className="inline-flex w-fit select-none items-center gap-1 rounded-xl border border-[var(--mnx-warning)]/20 bg-[var(--mnx-warning-bg)]/10 px-3 py-1.5 text-[10px] font-medium tracking-[0.08em] text-[var(--mnx-warning)] animate-pulse">
                                  STILL INSIDE
                                </span>
                              ) : (
                                <p className="text-xs font-medium leading-none text-mono-muted">
                                  —
                                </p>
                              )}
                              <p
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-mono-border/25 pt-3 text-[10px] font-medium text-mono-muted">
                          <span>Session {idx + 1}</span>
                          <span className="inline-flex select-none items-center rounded-full border border-[var(--mnx-accent)]/20 bg-[var(--mnx-accent)]/10 px-3 py-1.5 font-mono text-[10px] font-medium text-[var(--mnx-accent)] dark:text-[var(--mnx-accent)]">
                            {durationText}
                          </span>
                        </div>
                      </div>
