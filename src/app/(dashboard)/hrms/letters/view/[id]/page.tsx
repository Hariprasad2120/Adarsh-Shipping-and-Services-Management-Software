  ...
  if (!doc) {
    return (
      <div className="text-center py-20 text-mono-muted space-y-4">
        <AlertTriangle className="size-12 mx-auto text-[#D88700]" />
    ...
    <div className="space-y-6 animate-page-enter">
      ...
      <div className="relative rounded-3xl border border-mono-border bg-mono-card p-6 overflow-hidden shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="size-12 rounded-2xl bg-[#F9D972]/10 border border-[#F9D972]/25 flex items-center justify-center text-[#F9D972] shadow-sm">
          ...
          <div className="flex gap-3">
            {doc.pdfPath && (
              <a
                href={`/${doc.pdfPath}`}
                download
                className="inline-flex items-center justify-center gap-2 bg-[#F9D972] text-white hover:bg-[#E8C85D] hover:shadow-[0_0_0_3px_rgba(0,206,196,0.25)] px-4 py-2 rounded-xl text-xs uppercase tracking-wide font-bold transition-all"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="rounded-2xl border-mono-border/60 bg-mono-card shadow-sm">
            <CardHeader className="border-b border-mono-border p-4">
              <CardTitle className="text-sm font-bold uppercase tracking-wider flex items-center gap-2">
                <ShieldCheck className="size-4 text-[#F9D972]" />
            ...
            <CardContent className="p-4 space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="monolith-label block">Document Reference</span>
                  <span className="font-bold text-mono-text font-mono">{doc.letterNumber}</span>
                </div>
                <div className="space-y-1">
                  <span className="monolith-label block">Recipient Name</span>
                  <span className="font-bold text-mono-text">{doc.user.name}</span>
                </div>
                <div className="space-y-1">
                  <span className="monolith-label block">Issue Date</span>
                  <span className="font-bold text-mono-text">{doc.issuedAt ? new Date(doc.issuedAt).toLocaleDateString("en-IN", { dateStyle: "long" }) : "Pending"}</span>
                </div>
                <div className="space-y-1">
                  <span className="monolith-label block">Document Status</span>
                  <span className={`inline-block px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                    doc.status === "ACCEPTED" ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20" :
                    doc.status === "ISSUED" ? "bg-[#F9D972]/10 text-[#F9D972] border border-[#F9D972]/20" :
              ...
              {doc.documentHash && (
                <div className="space-y-1 pt-2 border-t border-mono-border/60">
                  <span className="monolith-label block">SHA-256 PDF Hash</span>
                  <span className="font-mono text-[10px] text-mono-muted break-all select-all">{doc.documentHash}</span>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-0">
              {doc.pdfPath ? (
                ...
                <div className="text-center py-20 text-mono-muted">
                  <FileText className="size-12 mx-auto text-[#F9D972]/30 mb-3" />
        ...
        <div className="space-y-6">
          <Card className="rounded-2xl border-mono-border/60 bg-mono-card shadow-sm monolith-card monolith-accent p-4 space-y-4">
            <h3 className="monolith-h3 text-sm font-black uppercase tracking-wider flex items-center gap-2">
              <Signature className="size-5 text-[#F9D972]" />
              Digital Acceptance
            </h3>
              ...
              <form onSubmit={handleAccept} className="space-y-4">
                ...
                <div className="flex items-start gap-2.5">
                  <input
                    ...
                    className="mt-0.5"
                  />
                  <label htmlFor="terms_check" className="text-[11px] text-mono-muted cursor-pointer select-none leading-normal">
                    I read, fully understand, and unconditionally accept the employment/internship terms and conditions specified.
                  </label>
                </div>

                <div className="space-y-1">
                  <label className="monolith-label">Type full name to sign</label>
                  <input
                    type="text"
                    required
                    value={nameSignature}
          ...
          <Card className="rounded-2xl border-mono-border/60 bg-mono-card shadow-sm p-4 space-y-3">
            <h4 className="text-xs font-black uppercase tracking-wider text-mono-text">Audit Trail Log</h4>
            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
              {auditTrail.map((log: any, idx: number) => (
                <div key={idx} className="border-l-2 border-[#F9D972]/40 pl-3 space-y-1 text-xs">
                  <div className="flex items-center justify-between text-[9px] text-mono-muted font-bold uppercase">
                    <span>{log.userName}</span>
                    <span>{new Date(log.timestamp).toLocaleDateString()}</span>
                  </div>
                  <p className="font-semibold text-[11px] uppercase text-mono-text">{log.action.replace(/_/g, " ")}</p>
                  <p className="text-[10px] text-mono-muted leading-relaxed">{log.details}</p>
                </div>
              ))}
            </div>
          </Card>
