...
import {
  ...
  updateAccountingCustomFieldDefinition,
...
export async function PATCH(
  ...
  try {
    ...
    return ok(
      await updateAccountingCustomFieldDefinition(
        session!.user.orgId!,
        id,
        parsed.data,
      ),
    ...
    return err(error instanceof Error ? error.message : "Unable to update field", 404);
  }
}
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await deleteAccountingCustomFieldDefinition(session!.user.orgId!, id);
    return ok({ deleted: true });
  } catch (error) {
    return err(error instanceof Error ? error.message : "Unable to delete field", 404);
  }
}
