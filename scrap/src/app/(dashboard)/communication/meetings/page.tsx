...
export default function MeetingsPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-0">
            ...
            {meetings.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border cursor-pointer flex justify-between items-start gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all duration-150 ${
                    isActive
                      ? "bg-[#00cec4]/10 text-[#00cec4] border-[#00cec4]/30 shadow-sm"
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="p-5 border-b border-[var(--color-outline-variant)] bg-[var(--color-surface-container-low)] flex justify-between items-center shrink-0">
              ...
              <a
                ...
                className="bg-[#00cec4] text-white hover:bg-[#00b8af] px-4 py-2.5 rounded-xl text-xs uppercase tracking-widest font-bold flex items-center gap-1.5 cursor-pointer relative transform-gpu transition-all duration-75 active:translate-y-[4px] active:shadow-[0_2px_0_0_#00857e] shadow-[0_6px_0_0_#00857e] border border-[#00b8af]"
              >
                <Video size={13} />
                Join Call Room
              </a>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="bg-[#00cec4] text-white hover:bg-[#00b8af] px-4 py-2.5 rounded-xl text-xs uppercase tracking-widest font-bold cursor-pointer relative transform-gpu transition-all duration-75 active:translate-y-[4px] active:shadow-[0_2px_0_0_#00857e] shadow-[0_6px_0_0_#00857e] border border-[#00b8af]"
                    >
                      Save Note
                    </button>
                  </div>
      ...
      <MeetingHumorLoader active={isPending} />

    </div>
  );
}

const HUMOROUS_MEETING_MESSAGES = [
  "Renting space on the Jitsi satellite...",
  "Calibrating webcam beauty filters...",
  "Stretching facial muscles for high-definition smiling...",
  "Drafting emergency excuse cards for mute button failure...",
  "Feeding hamster wheels for high-quality audio routing...",
  "Preparing coffee drops for virtual attendees..."
];

function MeetingHumorLoader({ active }: { active: boolean }) {
  const [msgIndex, setMsgIndex] = useState(0);

  useEffect(() => {
    if (!active) return;
    const interval = setInterval(() => {
      setMsgIndex((prev) => (prev + 1) % HUMOROUS_MEETING_MESSAGES.length);
    }, 2000);
    ...
    <div className="fixed bottom-6 right-6 z-50 animate-bounce transition-all duration-300">
      <div className="bg-[var(--color-surface)] border-2 border-[#00cec4] rounded-2xl p-5 shadow-[0_10px_0_0_#00857e] flex items-center gap-4 max-w-sm transform hover:scale-105 transition-transform duration-200">
        <div className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-[#00cec4]/10 text-[#00cec4] border border-[#00cec4]/30 shrink-0">
          <Video size={24} className="animate-spin text-[#00cec4]" />
