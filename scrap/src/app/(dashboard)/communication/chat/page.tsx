...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-0">
      ...
      <div className="w-full lg:w-64 shrink-0 flex flex-col bg-[var(--color-surface)] border border-[var(--color-outline-variant)] rounded-xl overflow-hidden min-h-0">
        <div className="p-4 border-b border-[var(--color-outline-variant)] flex justify-between items-center bg-[var(--color-surface-container-low)]">
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-[#00cec4]/15 border border-[#00cec4]/30 text-[#00cec4] hover:bg-[#00cec4]/25 hover:scale-105 active:scale-95 transition-all cursor-pointer"
          ...
          {conversations.map((convo) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-start gap-3 p-3 rounded-xl cursor-pointer text-left hover:scale-[1.02] active:scale-[0.98] transition-all duration-150 ${
                  isActive
      ...
      <ChatHumorLoader active={isPending} />

    </div>
  );
}

const HUMOROUS_CHAT_MESSAGES = [
  "Translating messages to binary slang...",
  "Running message through direct neural uplink...",
  "Evading chat moderation AI agents...",
  "Transmitting chat packets across dimensions...",
  "Formatting pixels for optimal sarcasm...",
  "Bribing fiber optic cables with electricity...",
  "Whispering message to the database listener..."
];

function ChatHumorLoader({ active }: { active: boolean }) {
  const [msgIndex, setMsgIndex] = useState(0);

  useEffect(() => {
    if (!active) return;
    const interval = setInterval(() => {
      setMsgIndex((prev) => (prev + 1) % HUMOROUS_CHAT_MESSAGES.length);
    }, 2000);
    ...
    <div className="fixed bottom-6 right-6 z-50 animate-bounce transition-all duration-300">
      <div className="bg-[var(--color-surface)] border-2 border-[#00cec4] rounded-2xl p-5 shadow-[0_10px_0_0_#00857e] flex items-center gap-4 max-w-sm transform hover:scale-105 transition-transform duration-200">
        <div className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-[#00cec4]/10 text-[#00cec4] border border-[#00cec4]/30 shrink-0">
          <MessageSquare size={24} className="animate-spin text-[#00cec4]" />
        ...
        <div>
          ...
          <p className="text-white text-xs font-semibold font-sans mt-0.5 animate-pulse">
            {HUMOROUS_CHAT_MESSAGES[msgIndex]}
          </p>
        </div>
      </div>
    </div>
