...
export default function CalendarPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          {events.map((ev) => (
            <div
              ...
              className="card-left-accent bg-[var(--color-surface-container-low)] border border-[var(--color-outline-variant)]/60 hover:border-[#00cec4]/40 rounded-xl p-4 space-y-3 cursor-pointer hover:translate-y-[-2px] hover:shadow-md transition-all duration-150 active:scale-[0.98]"
            >
        ...
        <div className="w-full lg:w-96 shrink-0 bg-[var(--color-surface)] border border-[var(--color-outline-variant)] rounded-xl overflow-hidden flex flex-col min-h-0">
          <div className="p-4 border-b border-[var(--color-outline-variant)] bg-[var(--color-surface-container-low)] flex justify-between items-center shrink-0">
            ...
            <button
              ...
              className="text-xs text-[var(--color-on-surface-variant)] hover:text-white uppercase font-bold cursor-pointer hover:scale-105 active:scale-95 transition-transform"
            >
              Close
            </button>
          </div>
            ...
            {selectedEvent.createdById === session?.user?.id && (
              <div className="pt-6 border-t border-[var(--color-outline-variant)]/40">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 text-xs uppercase tracking-widest font-bold bg-red-650 hover:bg-red-700 text-white border border-red-700 rounded-xl relative transform-gpu transition-all duration-75 active:translate-y-[4px] active:shadow-[0_2px_0_0_#7f1d1d] shadow-[0_6px_0_0_#7f1d1d] cursor-pointer"
                >
                  <Trash size={12} />
                  Delete Event
                </button>
      ...
      <CalendarHumorLoader active={isPending} />

    </div>
  );
}

const HUMOROUS_CALENDAR_MESSAGES = [
  "Synchronizing atomic clocks with Google Headquarters...",
  "Searching for wormholes in the spacetime continuum...",
  "Bribery negotiations with gravity for extra hours...",
  "Shoveling virtual coal into the scheduler engine...",
  "Chasing down runaway days of the week...",
  "Verifying that Friday actually falls after Thursday...",
  "Polishing the calendar grids...",
  "Banish mercury retrograde from the schedule..."
];

function CalendarHumorLoader({ active }: { active: boolean }) {
  const [msgIndex, setMsgIndex] = useState(0);

  useEffect(() => {
    if (!active) return;
    const interval = setInterval(() => {
      setMsgIndex((prev) => (prev + 1) % HUMOROUS_CALENDAR_MESSAGES.length);
    }, 2000);
    ...
    <div className="fixed bottom-6 right-6 z-50 animate-bounce transition-all duration-300">
      <div className="bg-[var(--color-surface)] border-2 border-[#00cec4] rounded-2xl p-5 shadow-[0_10px_0_0_#00857e] flex items-center gap-4 max-w-sm transform hover:scale-105 transition-transform duration-200">
        <div className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-[#00cec4]/10 text-[#00cec4] border border-[#00cec4]/30 shrink-0">
          <Clock size={24} className="animate-spin text-[#00cec4]" />
