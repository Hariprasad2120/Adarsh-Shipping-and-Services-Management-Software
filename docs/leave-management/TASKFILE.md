...
| 39 | Reporting gaps (10 more report types) | DONE (mostly) | 15 reports now built total. Added this round: Carry Forward, Encashment, Comp-Off, Accrual History, Balance Adjustments, Scheduler Runs. Reconciliation exists as its own dedicated tool (§30), not folded into reports.ts. Remaining not built: Compliance Exceptions, Expired Leave (distinct from Expiring), Policy Assignment |
...
| 45 | Frontend error states | DONE (real gap found+fixed) | Found: 3 admin pages (`hr-console`, `policies`, `team-calendar`) called `requirePermission()` (which throws `ForbiddenError`) directly in a server component with nothing catching it — an unauthorized visitor got Next.js's generic unstyled error boundary instead of a meaningful message. Fixed: switched to the boolean `can()` check + `WorkspaceState variant="danger"` (the repo's existing error-state component), matching the established pattern elsewhere. Form-level loading/error states (toast + disabled-while-submitting) were already correct across all leave client components — confirmed by inspection, not re-built |
