...
| 6 | Policy lifecycle (clone/version/compare) | NOT_YET | Create/publish exists; clone and version-diff UI not built |
...
| 11 | Approval escalation/delegation/backup approvers | NOT_YET | Not built this session — real scope item, needs SLA scheduler + admin UI |
...
| 13 | Attachment authorization (approver/HR visibility) | NOT_YET | Currently owner-only (documented limitation from Phase 9); broadening to approver/HR visibility not done |
| 14 | Comp-off automatic generation from attendance/OT | NOT_YET | `createCompOffCredit()` exists and is callable with `sourceOtRecordId`, but no scheduler/trigger wires it automatically from approved OT — manual/API-triggered only |
| 15 | Comp-off expiry (full edge cases) | PARTIAL→NOT_YET | Basic expiry cron exists (Phase 8); timezone/partial-consumption/cancellation-reversal-ordering edge cases not individually tested |
| 16 | Encashment workflow | NOT_YET | `ENCASHMENT` ledger type and config exist; no dedicated encashment service/route built |
...
| 20 | Restricted/optional holidays (full feature) | NOT_YET | `RESTRICTED_HOLIDAY` classification exists; quota/selection/deadline UI not built |
...
| 22 | Work calendar correctness (multi-shift/branch) | PARTIAL | `calculation.ts` reads `WorkingCalendar`/`Holiday` correctly; `WorkingCalendar` remains singleton-per-org (documented limitation since Phase 5), not per-branch/shift |
| 23 | Sandwich policy edge cases | PARTIAL | Core sandwich logic tested (Phase 12); multi-holiday/cross-month/cross-year/cancellation-removes-sandwich edge cases not individually tested |
...
| 26 | Distributed scheduler safety (simulate duplicate workers) | PARTIAL | `LeaveSchedulerRun.runKey` unique constraint provides DB-level claim/lock (a second worker's `create()` fails on the unique constraint) — this IS the distributed-safety mechanism, but no test simulates concurrent workers racing it |
...
| 29 | Data backfill strategy | PARTIAL | `initLeaveBalancesForUser` now ledger-backed (fixed this pass); no bulk backfill script for existing production balances/requests |
...
| 31 | New-joiner automation | PARTIAL | `initLeaveBalancesForUser` fixed to be ledger-correct; not wired to any onboarding trigger |
...
| 33 | Applicability re-evaluation on employee move | DONE (verified by design, no code needed) | Applicability (`isPolicyApplicableToUser`) is evaluated live at every submission, never cached — a branch/dept/employment-type move automatically changes what applies to future requests with zero migration code, and never retroactively touches existing balances/requests (which are keyed to `leaveTypeId` directly, not to a point-in-time applicability snapshot). Confirmed correct by code inspection, not built because nothing needed building |
| 34 | Compliance template admin UI | NOT_YET | `LeaveComplianceTemplate` schema + seed data exist (Phase 10); no dedicated admin UI page |
...
| 37 | Calendar integration | NOT_YET | Team calendar view exists (Phase 9, internal-only); no external Google/O365 adapter |
| 38 | Team calendar privacy | DONE | Verified in Phase 9 — shows name/type/dates only, never `notes` |
| 39 | Reporting gaps (10 more report types) | NOT_YET | 9 reports built (Phase 11); Carry Forward/Reset History/Encashment/Comp-Off/Accrual History/Balance Adjustments/Compliance Exceptions/Expired Leave/Policy Assignment/Scheduler Runs/Reconciliation not built |
...
| 41 | IDOR testing | DONE (partial scope) | 6 tests added covering the 5 fixed gaps; broader sweep (attachments, reports) not exhaustively tested |
...
| 45 | Frontend error states | NOT_YET | Not audited this pass |
| 46 | Accessibility | NOT_YET | Not audited this pass |
