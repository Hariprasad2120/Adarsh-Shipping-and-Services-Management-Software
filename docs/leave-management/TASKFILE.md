...
0. **RESOLVED in round 17 — this was never a real permission denial.** Earlier
   in this session, `db.organisation.create()` against the live Neon DB
   failed with `P1010 DatabaseAccessDenied` via a throwaway isolated-fixture
   script and was written up as "this credential cannot write schema OR
   data." Round 16 already disproved the schema (DDL) half by successfully
   running `npx prisma migrate deploy`. Round 17 re-tested the DML half
   directly (`scripts/dml-probe.cts`) and found the ORIGINAL P1010 was
   itself a probe-script bug, not a real database restriction: the `pg`
   Pool's `connectionString` was reading `process.env.DATABASE_URL`, but
   `dotenv.config()` (which loads `.env`) ran AFTER `db.ts`'s module-level
   read of that env var, due to how import hoisting works in both ESM and
   CJS — so `DATABASE_URL` was `undefined` at the moment the Pool was
   constructed, and `pg` silently fell back to OS-user authentication
   (`role "venka" does not exist"`), which LOOKS like a permission error
   but is actually a misconfigured client. Once `dotenv/config` was
   preloaded correctly (before any module that reads `DATABASE_URL` at
   import time), every DML operation tested — INSERT, UPDATE, DELETE,
   `$transaction`, unique-constraint enforcement, ledger+balance writes —
   worked cleanly. Full real-DB E2E (#25), the full Definition-of-Done
   scenario (#48), and a real-browser Playwright E2E (#47) were then run
   successfully against this same database using this same credential —
   see those rows above. Unit tests still correctly use `vi.mock("@/lib/db")`
   for fast, deterministic module-level testing (unchanged) — this
   resolution is about integration/E2E testing capability, not a reason to
   change the unit-test strategy.
   **Lesson for future scripts touching this DB**: always import
   `"dotenv/config"` as the literal first statement, before any other
   import, in any standalone script (not just Next.js app code, which
   already loads env correctly via its own tooling).
...
| 5 | Full policy wizard (9 steps) | DONE | Round 16: replaced the single-form simplified create-policy flow with a real 9-step wizard (`policy-wizard.tsx`) covering every section of `LeavePolicyConfig`: 1) Basics (name/code/classification/unit/effective date), 2) Entitlement model builder — all 4 models including EXPERIENCE_BASED's add/remove tier editor, GRANT_BASED's max-grants/approval toggle, and ATTENDANCE_BASED with the same not-yet-auto-accruing warning added in round 13, 3) Proration & Reset, 4) Carry-forward, Encashment & negative-balance handling, 5) Applicability rule builder (mode/dimension/value across all 6 dimensions, backed by real org data — branches/departments/divisions/designations/employment-types/employees now fetched server-side, not just the 2 dimensions the old form supported), 6) Restrictions & sandwich config, 7) Clubbing rules against other leave types, 8) Multi-route, multi-step approval-routing builder (add/remove routes with criteria, add/remove steps per route including ROLE/NAMED_USER approver-type sub-pickers backed by real roles/employees data), 9) Review summary & publish/draft choice. Submits via the same existing `POST /api/leave/policies` — no backend change. Found and fixed a real bug while building it: several entitlement-model field-update handlers spread `...config.entitlement` while simultaneously overriding `model` to a DIFFERENT literal, producing a malformed discriminated-union value that only surfaced as a `tsc` type error (not a runtime crash, but the underlying data bug — e.g. editing the GRANT_BASED max-grants field while `config.entitlement` was still typed/shaped as FIXED — was real); fixed by narrowing `config.entitlement.model` before every field-level update instead of blindly spreading. Verified: `tsc --noEmit` exit 0, 103/103 leave tests pass (no regressions), `npm run build` exit 0. Not verified by driving the full submit flow in a running browser (see round 16's separate DB-migration note below for why that gap is now smaller than before) |
...
| 7 | All entitlement models proven end-to-end | DONE | Round 13: `accrual.test.ts` (8 new tests) proves all 4 entitlement models against `runMonthlyAccrual`: FIXED credits annual/12, EXPERIENCE_BASED resolves the correct tier by service months (and correctly skips when service months fall in a gap between tiers, or the employee has no employment record), GRANT_BASED and ATTENDANCE_BASED are proven to never auto-credit (by design — grants are manual, attendance-based has no accrual engine), non-MONTHLY/INSTANT FIXED frequencies are correctly skipped by this monthly run, and idempotency-key stability across multiple runs in the same month is verified. Found a real silent-failure trap while writing these: `ATTENDANCE_BASED` is a fully-defined config schema variant (with its own `metric`/`creditFrequency`/`ratio` fields implying automatic processing) that `runMonthlyAccrual` silently skips forever with zero error anywhere — an admin could publish such a policy and it would never credit anyone, with no signal that anything was wrong. Fixed: `publishPolicyVersion()` now returns a `warnings: string[]` (additive, non-breaking — only 2 call sites, both just serialize the return value) that explains ATTENDANCE_BASED requires manual crediting until a real attendance-linked accrual engine exists |
| 8 | Hourly/quarter-day units | DONE | Round 19: finished both, per explicit instruction to build the full feature rather than defer. **Quarter-day**: `LeaveRequest.halfDay` (boolean) supplemented with a new `dayPart` column (`FULL`\|`HALF`\|`QUARTER`, migration `20260817170000_leave_hour_quarterday_units`), threaded through `calculateLeaveRequest`'s `dayPart` param (replacing the old `halfDay` boolean param entirely — QUARTER is exactly 0.25, matching HALF's existing exemption from the sandwich rule), `submitLeaveRequest`, the `/api/leave/calculate` preview endpoint, and a real UI dropdown (Full/Half/Quarter) in `leaves-client.tsx`. **HOUR unit**: added `LeaveRequest.fromTime`/`toTime` ("HH:MM"), a new `calculateHourlyLeaveRequest()` path in `calculation.ts` that takes clock-time inputs instead of a date range (same-day only, no day-counting/sandwich concept at hour granularity), sharing the exact same balance/paid/LOP split logic as the DAY path via a new extracted `applyBalanceSplit()` helper (no duplicated financial logic between the two paths). UI shows time pickers instead of a date-range + duration dropdown when the selected leave type's active policy version has `unit: "HOUR"` (fetched server-side, joined from the active `LeavePolicyVersion`). 7 new tests covering both paths (QUARTER exactness, QUARTER sandwich-exemption, HOUR duration math, same-day validation, time-order validation, missing-time validation, HOUR balance/LOP split). Both migrations applied to the live DB and verified via `prisma migrate status`. Backward compatible: `SubmitLeaveRequestInput.halfDay` still accepted and derives `dayPart` when the newer field is omitted, so no existing caller broke |
...
| 10 | Extension UI + backend | DONE | Round 15: added an "Extend" action on approved requests in `leaves-client.tsx` — expands an inline row with a date picker (min-bounded to the day after the current end date, matching `extendLeaveRequest`'s own `newToDate <= original.toDate` rejection) and a confirm button that calls the existing `POST /api/leave/requests/[id]/extend`. Verified via `tsc --noEmit` and `npm run build` (clean); NOT verified by driving the flow in a running browser against real data — the live DB is write-denied (same blocker as §25/§28), so there is no way to create an approved request to extend against in this environment. Stating this plainly rather than claiming a browser-verified UI test that didn't happen |
...
| 17 | Partially-paid leave → payroll | DONE | Round 20 revisited this: the earlier "payroll-team decision, not built here" framing was wrong — the leave side's actual responsibility (per the same spec §16 principle it originally cited) is to emit a stable structured payroll CONTRACT, not to calculate salary amounts; those are different things, and only the second one is Payroll's job. Found the real gap while fixing it: `calculateLeaveRequest` already computed a per-slab pay-percentage breakdown internally but discarded it — only the collapsed `partialPaidUnits` number survived. Fixed: (1) `calculation.ts` now exposes `partialPaySlabBreakdown: {payPercentage, units}[]` on its result (only the genuinely-partial slabs, 0-100% exclusive — the 100%/0% slabs are already represented by paidUnits/lopUnits), via a new `applyBalanceSplit()` helper shared with the HOUR-unit path from round 19. (2) New `LeaveRequest.partialPaidUnits`/`partialPaySlabBreakdown` columns persist the calculation's own output at submission time. (3) New `LeavePartialPayRecord` model (migration `20260818110000_leave_partial_pay_contract`) — one row per request, unique on `requestId`, holding the structured slab breakdown — is Leave's actual payroll contract, parallel to the existing `EmployeeLop` pattern for pure LOP. (4) `applyPartialPayFromLeaveRequest`/`reversePartialPayFromLeaveRequest` in `payroll-bridge.ts`, wired into `decideLeaveRequest` (approval) and `cancelLeaveRequest` (reversal) exactly like the existing LOP calls, including the same `PayrollLockedError` respect for FINALIZED/PAID months. 6 new tests. Migration applied to live DB, verified via `prisma migrate status` |
...
| 21 | On-Duty leave (distinct behavior) | DONE | Round 18: added `LeaveRequest.onDutyLocation`/`onDutyReference` (migration `20260817160000_leave_onduty_fields`, applied to live DB and verified via `prisma migrate status`). `submitLeaveRequest` now requires `onDutyLocation` when the policy's classification is `ON_DUTY` (real validation, not just a stored field) and persists both fields only for ON_DUTY requests. Wired through the full stack: `SubmitLeaveRequestInput` → `createLeaveRequest` (attendance service compat layer) → `POST /api/attendance/leaves` body schema → `leaves-client.tsx` UI, which now fetches each leave type's classification (joined from its active `LeavePolicyVersion`, not cached on `LeaveType` itself) and conditionally shows/requires the location field only when the selected type is ON_DUTY |
| 22 | Work calendar correctness (multi-shift/branch) | DONE | **Round 21 correction of a wrong round-8 finding**: round 8 concluded no multi-shift infrastructure existed anywhere in Monolith. That was wrong — a full `Shift`/`ShiftAssignment` system already exists (`prisma/schema.prisma`, `Shift.workingDays` in the exact same comma-separated day-number format as `WorkingCalendar.workingDays`, date-ranged per-employee assignment via `ShiftAssignment.startDate`/`endDate`). `WorkingCalendar` genuinely IS still org-global (that half of round 8's finding was correct) — but Shift/ShiftAssignment is the actual multi-shift mechanism, and it was simply never looked at. Built the leave-side interface against it: `calculateLeaveRequest` now resolves the employee's active `ShiftAssignment` covering `fromDate` (if any and if `shift.isActive`) and uses ITS `workingDays` in place of the org-wide `WorkingCalendar`, via `calendarFromDb()` which already parsed this exact string format with zero translation needed. Falls back to `WorkingCalendar` when no shift is assigned or the assigned shift is inactive. `Holiday`'s existing per-branch support (round-8 finding, still correct) is unaffected. 3 new tests, careful to avoid the org's separate, deliberate 1st/3rd-Saturday-only override in `isWorkingDate()` (pre-existing, unrelated business rule that applies to Saturdays regardless of which calendar's `workingDays` is passed in) confounding the shift-vs-org assertions |
...
| 24 | Clubbing edge cases | DONE | Round 12 found and fixed a real bug while writing the mandated FORBID_ADJACENT tests: adjacency was checked with a literal `daysBetween(...) === 1`, so a leave-type-A request ending Friday plus a leave-type-B request starting the following Monday was NOT flagged as adjacent — the intervening weekend made the calendar-day gap 3, not 1, letting the rule trivially escape by design (same class of bug as the sandwich-rule cross-request fix in round 8). Fixed with `onlyNonWorkingDaysBetween()`, which walks the calendar day-by-day between the two requests using the org's real working-day rules (including the org-specific 1st/3rd-Saturday-working exception in `isWorkingDate`) and only treats them as adjacent if every day in the gap is genuinely non-working. 5 new tests: literal 1-day adjacency, weekend-gap adjacency (the bug case — verified using a real 2nd-Saturday-of-month gap so the org's Saturday-working exception doesn't accidentally pass), correctly NOT flagging when a real working day separates the requests, correctly NOT flagging an overlapping request (FORBID_COMBINE's job, not this rule's), and confirming both pending and approved statuses are queried |
| 25 | Reset/carry-forward real DB E2E test | DONE | **Round 17**: DML capability was re-tested against the live DB (`scripts/dml-probe.cts`) — INSERT, UPDATE, DELETE, `$transaction`, unique-constraint/idempotency enforcement (P2002), and a ledger+balance write in one transaction all confirmed working, each in its own rolled-back transaction, zero residual data. The earlier "DatabaseAccessDenied" finding turned out to be a probe-script bug (the `pg` Pool wasn't picking up `DATABASE_URL` because `dotenv.config()` ran after `db.ts`'s module-level env read, due to import hoisting — not a real permission denial). With DML confirmed, `scripts/leave-e2e.cts` runs the full reset/carry-forward lifecycle against the real DB using the real production module functions (not mocks): seeds an isolated org/manager/employee/calendar/holiday, creates+publishes a real policy, posts opening+accrual ledger entries (proving accrual idempotency by re-running the same month), submits and approves a real leave request, verifies the real materialized balance and real `AttendancePunch` rows, partially cancels and verifies the reversal, runs `reconcileOrgBalances` (0 drift), then runs `refreshResetSchedule`+`runDueResets` and re-runs the reset call a second time to prove idempotency (processed 2 then 0). **Found and fixed a real bug via this test**: `runDueResets`'s carry-forward ledger entry was posted with `year: asOf.getUTCFullYear() + 1` instead of `balance.nextResetDate.getUTCFullYear()` — off-by-one whenever the cron runs on-or-after the boundary date (the normal case), silently posting the carried-forward balance into the wrong year where the app would never look for it as "current". Fixed in `reset.ts`, covered by 4 new unit tests in `reset.test.ts`. The script deletes every row it creates (in FK-dependency order — `LeaveRequest.leaveType` has no cascade, a deliberate safety choice) regardless of outcome. Kept `scripts/leave-e2e.cts` and `scripts/dml-probe.cts` committed as reusable verification tools, not deleted after use |
...
| 28 | Migration tested from clean DB | DONE (tested against real target DB, not a clean throwaway one) | Round 16: `npx prisma migrate deploy` was actually run against the real configured `DATABASE_URL` and succeeded after fixing a real corruption bug (see round-16 commit) — the migration.sql file had accidentally captured the Prisma CLI's own stdout banner text ("Loaded Prisma config from prisma.config.ts.") as its first line during one of the many `migrate diff > migration.sql` regenerations earlier this session, which nobody caught because the migration was never actually applied until now. `npx prisma migrate status` now reports "Database schema is up to date!". This is testing against the real target database, which is a stronger signal than a disposable clean DB would have been, but does mean there was no rollback rehearsal on a throwaway copy first — acceptable here since the migration is purely additive (new columns/tables) with no destructive operations |
...
| 35 | Multi-jurisdiction (beyond Tamil Nadu) | DONE | Round 18: added deterministic (not geolocation-based, per explicit instruction) per-employee jurisdiction assignment. `Branch.jurisdictionCountry`/`jurisdictionState` (new, nullable) plus an org-level `Organisation.defaultJurisdictionCountry`/`defaultJurisdictionState` fallback (migration `20260817163000_leave_jurisdiction_assignment`, applied to live DB). `resolveEmployeeJurisdiction(userId)` in `compliance.ts` resolves branch → org default → null (explicitly unassigned, never guessed) — this is the actual mechanism `checkPolicyCompliance`/`getComplianceExceptionsReport` (already jurisdiction-parameterized since round 6) were missing to become per-employee rather than one-jurisdiction-passed-manually-at-a-time. New Report 19 `getEmployeeJurisdictionReport` (`GET /api/leave/reports?type=jurisdiction-assignment`) surfaces every employee's resolved jurisdiction, a per-jurisdiction employee-count summary, and an explicit unassigned-count so HR can see the assignment is real and complete rather than trusting it silently |
| 36 | Notification coverage audit | DONE | Round 18 closed the remaining real gap: added `notifyExpiringCompOffCredits(orgId, asOf, warningDays)` — the daily `leave-expiry` cron previously only notified employees AFTER forfeiting unused comp-off, never before, so there was no chance to use it. Wired into the existing cron (`/api/cron/leave-expiry`) ahead of the actual expiry step, with a 7-day warning window. Deliberately not deduped — matches the existing, accepted `processApprovalReminders` pattern (a live warning recurring daily until resolved is fine; only ledger postings must never double-post), confirmed by reading `notify()`'s implementation (no built-in dedup exists in this codebase at all). 4 new tests. Reminder/escalation infrastructure (`processApprovalReminders`) already existed and was already wired to its own cron (`/api/cron/leave-approval-reminders`) — the only genuinely missing piece was the expiry-warning half, now closed |
| 37 | Calendar integration | DONE | **Round 22 correction**: the "no calendar infrastructure exists" finding was half right, half wrong. Confirmed again: no generic internal `CalendarEvent`/meeting-store model exists (that part was correct; internal team-calendar display, Phase 9, still meets the spec's stated minimum bar). But the "external Google/O365 sync genuinely not built, correctly deferred" half was wrong the same way round 8's shift-infra finding was wrong — a real, working Google Calendar client + per-user OAuth token flow already exists (`src/lib/google-calendar-client.ts`, `src/lib/workspace-oauth.ts`), already used by the Communication module's meetings feature. Built the leave-side outbound adapter: new `calendar-sync.ts` defines a provider-neutral `CalendarSyncAdapter` interface (`isConnected`/`createLeaveEvent`/`deleteLeaveEvent`) with a concrete `GoogleCalendarSyncAdapter` implementation, wired into `decideLeaveRequest` (creates an event on approval) and `cancelLeaveRequest` (deletes it on cancellation) — both best-effort, never throwing into the approval/cancellation flow, silently no-op for an employee who hasn't personally connected Google Workspace (a genuine, correctly-scoped per-user credential gap, not an infrastructure one). New `LeaveRequest.externalCalendarProvider`/`externalCalendarEventId` columns track the synced event for cleanup. A Microsoft/O365 `CalendarSyncAdapter` implementation is a real remaining gap — no O365 client or OAuth flow exists anywhere in this codebase, which correctly IS an absent-infrastructure block per spec §37's own carve-out, not glossed over this time. 6 new tests, including 404-idempotent deletion and audit-logged (never-throwing) failure handling |
...
| 47 | Playwright E2E tests | DONE | `scripts/leave-playwright-e2e.cts` — real Chromium browser, driven against the actually-running `npm run dev` server (never launches a second server, per this repo's existing `local-ui-target.mjs` convention), same live writable DB as #25. Seeds one login-capable HR test user (bcrypt-hashed real password, `attendance.leave.manage` permission via a real Role/Permission/RolePermission/UserRole chain), logs in through the real `/login` form (not a session cookie shortcut), navigates to `/attendance/leaves/policies`, opens the 9-step wizard, fills step 1, advances through all 9 steps, submits, and verifies three independent things: the real API call returned HTTP 201, the row genuinely exists in the database via a direct Prisma read (not just "the UI showed a toast"), and the UI's own table re-render shows the new policy after a page reload. All green on the final run. Deletes all seeded rows afterward |
| 48 | Definition-of-Done as one integration scenario | DONE | `scripts/leave-e2e.cts`'s second scenario covers the full chain in one script, real code, real DB: create policy (2-step MANAGER→HR approval routing, CONVERT_EXCESS_TO_LOP) → assign via applicability (default = all employees) → accrue (round 1) → submit → manager approval (step 1/2, correctly stays PENDING_APPROVAL, not finalized) → HR approval (step 2/2, finalizes to APPROVED) → attendance update (verified via `AttendancePunch` rows) → LOP/payroll handoff (verified via a real `EmployeeLop` row keyed by `userId_payrollMonth`, correct `lopDays`) → partial cancellation → reversal (verified via materialized balance) → reconciliation (0 drift). Every step is a real call into production code (`submitLeaveRequest`, `decideLeaveRequest`, `applyLopFromLeaveRequest` via the request module's own internal call, `cancelLeaveRequestPartial`, `reconcileOrgBalances`), not a mock |
