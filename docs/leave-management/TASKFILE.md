...
0. **Live DB row-level writes (DML) were denied earlier in this session; schema writes (DDL) were confirmed working in round 16 — the two are NOT the same permission and this blocker's original wording overstated the scope.**
   ...
   `DATABASE_URL` credential appeared to lack INSERT grants at the database
   level. At the time this was written up as "this credential cannot write
   schema OR data" — that combined claim was never actually tested for the
   schema half.
   **Round 16 correction**: the user hit a live "column does not exist"
   runtime error, which led to running `npx prisma migrate deploy` against
   this exact same `DATABASE_URL` — it succeeded (after fixing an unrelated
   migration-file corruption bug, see round 16 commit). So DDL (schema
   changes) demonstrably works with this credential; only DML (row-level
   INSERT/UPDATE/DELETE) was ever confirmed denied, and that confirmation is
   now over a session old and was not re-tested in round 16.
   **Resolution (unchanged)**: unit tests still use `vi.mock("@/lib/db")`,
   this repo's established convention, since even an optimistic re-test of
   DML doesn't change that mocked unit tests are the right tool for fast,
   deterministic module-level tests regardless of DB access.
   **What a human should do**: re-test whether DML now works (a single
   throwaway `db.organisation.count()` read plus a scoped test-row
   create/delete would confirm it cheaply) before assuming real E2E/DoD
   integration tests are still blocked — the earlier "confirmed, not assumed"
   framing for DML denial is now itself close to a year-old assumption in
   session-time and deserves a fresh check, not a repeat of the same claim.
...
| 25 | Reset/carry-forward real DB E2E test | BLOCKED_EXTERNAL (partially re-scoped, round 16) | **Update**: the user hit a live "column does not exist" error running `npm run dev` against the real DB, which led to discovering the leave-management migration had never been deployed — `npx prisma migrate deploy` was run this round and succeeded (see round-16 commit for the corrupted-migration-file bug that caused the first attempt to fail, and how it was fixed and re-applied). Schema DDL access to this DB is therefore confirmed working, contrary to the blanket "write-denied" characterization from earlier in this session. However, that earlier finding was specifically about row-level DML (`db.organisation.create()` returned `P1010 DatabaseAccessDenied`) — DDL and DML frequently have different grants in Postgres, and DML was NOT re-tested this round (out of scope for the immediate fix; re-testing it is the natural next step, not assumed here). Until DML is confirmed, a true E2E test that inserts/updates rows remains blocked; unit-level logic stays mocked-tested (Phase 12) |
