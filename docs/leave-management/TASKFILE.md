...
| 11 | Approval escalation/delegation/backup approvers | PARTIAL (delegation DONE, escalation NOT_YET) | Backup-approver delegation implemented: `LeaveApproverDelegation` model, `src/modules/leave/delegation.ts` (`createDelegation`/`revokeDelegation`/`resolveActiveApprover`), wired into `buildApprovalSteps` so a delegate is substituted at step-creation time. Routes: `POST /api/leave/delegations`, `GET /api/leave/delegations`, `POST /api/leave/delegations/[id]/revoke`. SLA reminders/escalation still not built — needs a scheduler job, not just a data model |
