...
| 8 | Hourly/quarter-day units | PARTIAL (scope boundary identified, user-confirmed) | Round 14 found `LeavePolicyVersion.unit` ("DAY"\|"HOUR") is stored but never read anywhere in `calculateLeaveRequest` — zero hour-granular duration logic exists; the only sub-day granularity implemented is the boolean `halfDay` flag (always exactly 0.5) plus whatever `roundingIncrement` produces on top of whole/half-day counts. Added 3 new end-to-end tests proving what DOES exist actually works through the full calculation pipeline (not just the standalone `applyRounding` unit): half-day correctly draws 0.5 from balance (not a full day), a clean 0.25 increment survives NEAREST rounding unchanged, and a non-0.25-aligned increment (0.3) correctly rounds via `Math.round(steps)`. True hour-based or quarter-day-as-a-first-class-unit support needs `LeaveRequest.halfDay` to become a fractional/enum field (schema migration) + calculation-engine + UI changes — asked the user directly rather than guessing scope; user chose "test what exists + document the gap" over building the feature this round |
