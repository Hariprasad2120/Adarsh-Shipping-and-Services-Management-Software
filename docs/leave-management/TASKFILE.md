...
| 39 | Reporting gaps (10 more report types) | DONE | 18 reports built total, covering every named type from spec §35's list. This round added: Expired Leave (distinct from forward-looking Expiring), Policy Assignment (live applicability evaluation per employee per policy), Compliance Exceptions (aggregates checkPolicyCompliance across every published policy for a jurisdiction). Reconciliation remains its own dedicated tool (§30) by design, not folded into reports.ts |
