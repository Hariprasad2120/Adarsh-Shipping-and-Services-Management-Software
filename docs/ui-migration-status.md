...
## 2026-08-03 HR letters draft submission workflow fix

Closed a workflow gap in the live HR Letters workspace where newly created
drafts stayed trapped in the registry because the UI exposed no action to move
 them into the review pipeline.

Delivered:

- updated `src/modules/hrms/components/letters-view.tsx` so draft rows in
  `Letters Registry` now expose a `Submit for Review` action;
- updated the letter review modal so a draft opened from the registry can also
  be submitted directly without leaving the detail view;
- preserved the existing approval flow after submission, so HR, Legal,
  Management, and Issue actions still appear only at their intended statuses.
...
- `NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit --pretty false`:
  failed due to pre-existing repository-wide TypeScript errors in unrelated
  Accounting, CRM, and Incentives files;
- `NODE_OPTIONS=--max-old-space-size=8192 npm run architecture:check`:
  failed due to pre-existing tracked generated artifacts under
  `artifacts/ui-migration/final-runtime/*`.
